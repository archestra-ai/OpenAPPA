//! The Archestra adapter: the server-side derivation the runtime applies
//! to every call Archestra's LLM proxy hands it. Pure — no policy, no
//! state, no runtime calls; the compiler enforces the boundary, since
//! this crate depends only on `appa-runtime-api`.
//!
//! There is no client-side codec and no wire. Archestra embeds the
//! runtime in its own process and builds each `HookEvent` itself, so it
//! derives every call through [`adapter`] before handing the event over,
//! the way the wire derives a served host's calls: the canonical identity
//! comes from the raw spelling alone, and the runtime spells a tool back
//! to the model through the derivation's inverse.
//!
//! Archestra names a tool `<catalog>__<tool>`: the slug of the MCP
//! catalog the tool was installed from, two underscores, the tool's own
//! name. Its parser splits at the *last* `__`, because a catalog slug may
//! itself contain one, and this adapter splits the same way. The mapping
//! onto canonical identity, a bijection over the spellings it accepts:
//!
...
//!
//! The third row also takes a name whose catalog segment would contain
//! `__` — `my__cat__tool` — so such a call is still an identity a root
//! rule can name by its raw spelling, while no battery rule under `mcp/`
//! reaches it. A character outside the segment grammar is refused and
//! the call blocks.
//!
//! Archestra's delegation is its own: a child trajectory binds to its
//! spawn through the event the host names it in, never through an
//! argument the parent spells, so [`names_children`] is always empty and
//! no raw spelling derives as the spawn.

use appa_runtime_api::{Actor, Adapter, AdapterName, CanonicalTool, Derived, ParseRefusal, ProposedCall, TrajectoryId};

/// The server-side derivation the runtime applies to every Archestra call.
pub fn adapter() -> Adapter {
    Adapter {
        name: AdapterName::Archestra,
        derive,
        names_children,
        spell,
    }
...
pub const CONTROL_TOOL_RAW: &str = "archestra__execute_remedy_plan";
...
mod tests {
    ...
    #[test]
    fn the_adapter_serves_archestra() {
        assert_eq!(adapter().name, AdapterName::Archestra);
    }
    ...
    fn each_raw_spelling_maps_onto_its_canonical_identity_and_back() {
        for (raw, expected) in [
            ...
            ("github_prod__get_file_contents", "mcp/github_prod/get_file_contents"),
    ...
    fn a_spelling_outside_the_grammar_is_refused() {
        for raw in ["", "github__", "with space", "github/get", "a__b__", "mcp:github/x"] {
    ...
    fn no_call_names_children() {
        ...
        let call = ProposedCall {
            ...
            arguments: serde_json::value::RawValue::from_string(r#"{"path":"tasks/a1.output"}"#.into()).unwrap(),
        };
