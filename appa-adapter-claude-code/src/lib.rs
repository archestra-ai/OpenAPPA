...
//! | `PostToolUse` for `Agent` (`Task`) | `SpawnResult` when the response carries the subagent's message (`content`); `ToolResult` when it is the launch acknowledgement |
...
//! fork still open for binding. The child's `SubagentStop` is the return
//! channel: its `last_assistant_message` is the message the parent
//! receives, and the codec reports it as `ChildEnd` naming the child.
//! A `decision: block` answer there keeps the subagent running with the
//! reason, and it stops again (`stop_hook_active: true`), so a return
//! that may not cross holds the subagent until it returns an admissible
//! message. The parent's `Agent` `PostToolUse` is the spawn outcome
//! only. In a top-level session the spawn is asynchronous: that hook
//! fires at launch with `agentId` and no `content`, and crosses as a
//! plain `ToolResult`. A synchronous spawn (`claude -p`) delivers the
//! same message in `content` after the child's stop and crosses as
//! `SpawnResult`, which replays the crossing the child's stop decided.
//! Claude Code's own helper agents stop with an empty `agent_type`, no
//! `SubagentStart` and no tool calls: their stop is the child's
//! `TurnEnd`, and no return is claimed. No hook can substitute what the
//! parent receives, so a return substitution (`ChildReturn`) is not
//! enforceable on this harness: the codec answers it with a block, and
//! the subagent must return a message that crosses unchanged.
//!
...
const UNSUBSTITUTABLE_RETURN: &str =
    "[appa] the return cannot be substituted where the parent receives it; return a message that may cross unchanged";
...
impl WireEvent {
    ...
    /// The spawn's result when the parent's `Agent` response carries the
    /// subagent's message (`content`); a launch acknowledgement carries
    /// none and is a plain tool result.
    fn spawn_return(&self) -> Option<(Option<TrajectoryId>, Option<String>)> {
        ...
        let value = match text_blocks(content) {
            Some(texts) => texts.join("\n"),
            None => content.to_string(),
        };
        Some((child, non_empty(Some(&value)).map(str::to_string)))
    }
}
...
fn render(event: &HookEvent, decision: &HookDecision) -> serde_json::Value {
    match decision {
        ...
        // Where the codec cannot restate what the parent receives, the
        // substitution is not enforceable: the return stays withheld.
        HookDecision::ChildReturn { value } => match replacement(event, value) {
            Some(replacement) => replaced(replacement, None),
            None => block(UNSUBSTITUTABLE_RETURN),
        },
...
mod tests {
    ...
    #[test]
    fn a_launch_acknowledgement_is_the_spawns_tool_result() {
        ...
        for (tool, response) in [
            ...
            assert_eq!(
                ...
                "the {tool} launch acknowledgement is a plain tool result",
            );
        }
    ...
    fn every_child_end_decision_renders_its_exact_wire_body() {
        ...
        assert_eq!(
            ...
            serde_json::json!({"decision": "block", "reason": UNSUBSTITUTABLE_RETURN}),
            "a substitution the harness cannot apply stays withheld",
        );
        assert_eq!(
            render(
                &event,
