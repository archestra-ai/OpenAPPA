...
impl Session {
    ...
    /// Check one proposed call without executing it. The framework must execute an allowed call
    /// exactly once and pass its real outcome to `report` before checking another call.
    ...
    /// Report the real outcome of the one call allowed by `check`.
    ...
    /// Close an allowed call as indeterminate when the framework aborts before observing a result.
