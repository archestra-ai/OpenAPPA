...
/// The largest upstream completion body read by default.
...
/// The base URL preceding `/chat/completions`, commonly ending in `/v1`.
...
/// The provider-specific model identifier pinned on every request.
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// Why an upstream inference round failed. Every variant is fail-closed.
...
impl OpenAiCompatible {
    ...
    /// Build with an existing redirect-disabled runtime client.
    ...
    /// Run one provider call. The configured model replaces any model in `request`.
