...
async fn serial_fork_joins_only_the_admitted_child_return_and_an_outcome_specific_fork_response() {
    ...
    // The child's context is exactly the ancestor's completed prefix plus its own errand. The
    // driver appends a finishing note to that errand, so provenance is asserted, not its wording.
