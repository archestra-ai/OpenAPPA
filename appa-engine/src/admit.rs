...
//! value is admitted raw (at the contract's output label), cast-resolved (a confined pending-cast
//! result whose Unknown dimension a registered cast established), or not at all (oversized). The
//! **label folds only from an admitted value**, never from the close itself.
//!
...
mod tests {
    ...
    fn pending_cast_confines_raw_admission() {
        ...
        let p = views_of(&log);
        let t = traj();
        // No value may enter carrying an unestablished label.
        assert_eq!(
            admit_result(
                &reg,
                &p.view(&t),
