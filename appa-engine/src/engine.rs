...
//!
//! It owns no state and performs no IO. Each method takes the current [`Views`] and returns a
//! decision and/or a validated [`FactBatch`] the runtime appends against the views' revision.
...
/// Why an engine operation could not proceed on the call as given.
...
/// The pure decision core, owning the immutable registry (its static capability).
#[derive(Clone, Debug)]
pub struct Engine {
    registry: Registry,
}
...
impl Engine {
    ...
    /// dimension no cast has established (`CHK-1`). Resolution is the runtime's job (`CHK-16`);
    ...
    /// return can merge (`BRN-14`). Policy-independent: the runtime drives resolution *before*
...
mod tests {
    ...
    fn clean_call_allows() {
        ...
        // Public+trusted trajectory; get_ticket requires trusted (met) but narrows to internal.
        let log = vec![user_value(known(TRUSTED, Audience::Public))];
        // Narrowing present → Block (soft), not Allow.
    ...
    fn repeat_at_same_label_is_not_a_narrowing() {
        let e = engine(vec![crm_tool()]);
        // Already internal + trusted: get_ticket's delta (internal) leaves the label unchanged.
    ...
    fn unknown_label_is_unestablished_not_a_gap() {
        let e = engine(vec![crm_tool()]);
        // A value with Unknown trust makes the branch fold Unknown: the consuming trust floor
        // lands in the block's `unestablished` slot per value, and its gap evaluation is masked —
        // a missing fact is never also a coverable gap. The call's own audience delta still fires
        // the narrowing clock, and the block carries both slots at once (`CHK-1`).
    ...
    fn all_three_block_slots_coexist() {
        // One block, everything that stopped the call at once (`CHK-1`): the attention demand is
        // a real gap, the call's own audience delta fires the narrowing clock, and the consumed
        // Unknown trust is named `unestablished` — none of the three hides another.
    ...
    fn an_unknown_trajectory_blocks_only_requirement_consuming_calls() {
        ...
        // dimension blocks with exactly the consumed dimension's values named `unestablished`; a
    ...
    fn open_dispatch_refuses_a_blocked_call() {
        // get_ticket requires trusted; at suspicious the check blocks, so open_dispatch must refuse
        // rather than emit an appendable dispatch for a call it would not allow.
        let e = engine(vec![crm_tool()]);
        let internal = Audience::restricted([ReaderId::new("internal")]);
        let log = vec![user_value(known(SUSPICIOUS, internal))];
        let p = Projection::build(&log, Revision::new(log.len() as u64));
    ...
    fn includes_missing_placeholder_fails_closed_on_public() {
        // A public trajectory with includes($to) and no `to` argument must block, not pass vacuously.
        let send = ToolContract {
            name: ToolName::new("send_email"),
            tags: vec![],
            delta: Some(Delta::NONE),
        ...
        // On an Unknown audience the malformed placeholder consumes the dimension and its
        // sentinel gap is masked: reported as a gap too, a reader-ceiling authority could cover
        // it and open the dispatch with the Unknown never resolved.
