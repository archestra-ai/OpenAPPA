...
//!
//! A [`Label`] is the product of two independent dimensions — [`Trust`] and [`Audience`] —
//! each wrapped in [`Dim`] so "not yet established" ([`Dim::Unknown`]) is a first-class state,
//! distinct from every rank and every reader set. Propagation folds the label *restrictively*
//! ([`Label::combine`]: minimum trust, intersect audience) — the label only ever moves down and
//! there is no permissive delta representable. Checking is the sink-side adequacy relation
//! ([`Adequacy`]), kept strictly separate from the fold: propagation lowers, checking decides.
...
/// Which label dimension a fact or requirement concerns.
...
///
/// The chain's names (`suspicious < trusted`, or a deployment's own longer chain) are
/// configuration; the pure algebra carries only the rank. Folding takes the minimum — one low
/// read lowers the whole trajectory — and a floor requirement accepts any rank at or above it.
...
impl Trust {
    ...
    /// The restrictive fold on trust: keep the worse (lower) rank.
...
impl Audience {
    ...
    /// The restrictive fold on audience: intersect — only readers cleared for every input remain.
    fn combine(&self, other: &Self) -> Self {
        match (self, other) {
    ...
    /// `self ⊆ cap` — the trajectory's readers stay within the tool's declared cap.
...
/// Fold one dimension, propagating `Unknown` as absorbing: two `Known`s combine via `f`, and any
/// `Unknown` makes the fold `Unknown` (see [`Dim`]).
...
impl Dim<Audience> {
    /// Does the trajectory's audience include `recipients` (`audience ⊇ recipients`)?
    ...
    /// Does the trajectory's audience stay within `cap` (`audience ⊆ cap`)?
...
mod tests {
    ...
    /// Generate `Unknown` as its own variant — never as a trust rank (there is no rank for it).
    ...
    proptest! {
        ...
        /// The restrictive law: on established dimensions the fold never widens — trust cannot
        /// rise and the reader set cannot grow.
        #[test]
        ...
        /// `top()` is the fold identity: folding it in changes nothing.
        #[test]
        fn top_is_identity(a in label_strategy()) {
            prop_assert_eq!(Label::top().combine(&a), a.clone());
        }

        /// Unknown is absorbing: folding a Known dimension with an Unknown one yields Unknown.
        #[test]
