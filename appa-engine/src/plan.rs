...
/// The atoms executing one offered plan reads: each assigned authority's mandate over
/// its covered gaps. The call's contract is evaluated separately, so symbolic comparisons
/// do not force a directory lookup here. A step's sanitizer
