...
impl Registry {
    pub fn build(config: RegistryConfig) -> Result<Registry, LoadError> {
        ...
        // Bound `remedy_plans` enumeration per tool (`RMD-5`). Only the executable kind needs a
        // cap: its worst case is a *product* over the competent authorities of every unmet
        // requirement entry, so it grows combinatorially. Static except `includes`, whose
        // recipients are call arguments — upper-bounded by every scope-covering authority with a
        // reader ceiling. Refused at load, so enumeration never truncates at runtime.
        //
        // The id-less redispatch kind needs none. It is linear: at most one entry per registered
        // tool, which the registry itself bounds, and the two kinds never appear together — a
        // prior/cap gap has no covering mandate, so a block carrying one has no executable plan.
        // A cap over the tool count would refuse ordinary catalogues (bench-corp registers 17)
        // without any enumeration blow-up to prevent.
