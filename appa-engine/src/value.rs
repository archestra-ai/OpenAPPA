...
//!
//! Data flows as a [`LabeledValue`] — a body plus its [`Label`], never separated. A proposed tool
//! call is resolved into an immutable [`ResolvedCall`] carrying a [`CanonicalDigest`] over the tool
//! name and resolved arguments; a dispatch of that call is identified by a [`DispatchId`]. The
//! digest is what a later ruling is scoped to, so approving `transfer(A, $1)` cannot authorize
//! `transfer(B, $100)`: the two render to different digests.
...
/// A tool's name — routing identity, never algebraic.
...
/// A trajectory (one branch of one agent run) — the server-minted key of an event log.
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
...
impl CanonicalDigest {
    /// Digest the canonical form of a call. Arguments serialize with sorted keys (serde_json's
    /// default map is order-independent), so equal calls always hash equally.
...
pub enum Provenance {
    /// The trust boundary: a user turn admitted at the north face under server policy.
    UserInput,
    /// The result of a successful tool dispatch.
    ToolResult { dispatch: DispatchId },
    /// A new value derived from an existing one by a registered transform (sanitizer/cast/
    /// authority raise). The source is never mutated — derivation mints a fresh value.
    Derived { source: ValueId, via: Derivation },
    /// A value a child branch returned, absorbed into the parent at merge (server-derived label).
    ChildReturn { child: TrajectoryId, id: ChildReturnId },
}

/// The registered transform that produced a [`Provenance::Derived`] value.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
/// A body and its label, inseparable — the unit the engine folds and checks.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
mod tests {
    ...
    fn digest_is_deterministic_and_key_order_independent() {
        ...
        // Same tool + same arguments (regardless of key order or arg_refs) → same digest.
