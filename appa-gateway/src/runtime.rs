...
/// Shared resource ceilings for one root run and its serial descendants.
...
/// The HTTP adapter's shared state.
pub struct Runtime {
...
impl Runtime {
    /// `head` is this host's transcript head (`CFG-18`) — the system/developer messages opening
    ...
    ///
    /// The live agent computes exact fork availability from session depth and the shared budget.
