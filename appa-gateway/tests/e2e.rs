...
async fn a_returned_child_session_refuses_a_new_turn_with_conflict() {
    // A value return ends the errand: driving the returned child session again is the caller's
    // lifecycle error — 409, never an internal fault, and no model request is made for it.
    let (model, seen) = spawn_scripted_model(vec![
        final_round("1", "parent ready"),
