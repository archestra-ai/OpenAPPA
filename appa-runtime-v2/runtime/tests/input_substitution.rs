...
use appa_runtime_v2::api::{AuditEvent, DispatchOutcome, OfferId, RemedyOutcome, Runtime};
...
async fn the_replaced_call_runs_through_the_hooks_and_closes() {
    ...
    let RemedyOutcome::Substituted { call } = runtime.execute_remedy(hop.clone()).await else {
    ...
    assert!(matches!(
        runtime.execute_remedy(hop).await,
