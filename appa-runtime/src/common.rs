...
/// A result admission's engine-side verdict. `Admitted(None)` is a value-less close.
...
/// A dispatch that no longer matches its own call/branch — an SDK invariant breach, surfaced loudly.
...
impl Core {
    ...
    pub(crate) async fn resolve_remedy(&mut self, plan_id: Option<&str>) -> Result<Remedied, StoreError> {
        ...
        // The rulings realize exactly the chosen plan's grouped assignment (its `required`), in
        // lockstep with the runtime's authority path.
        ...
        for req in &chosen.required {
            ...
            // Fail-closed on an unresolvable argument reference (unreachable while SDK calls carry
            // no refs — kept in lockstep with the runtime's authority path).
...
/// Map a tool outcome to the engine admission (v0: no sanitizer/cast paths).
...
/// Reject every policy feature the SDK v0 defers.
