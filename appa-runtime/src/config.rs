...
/// The spec default trust chain (`suspicious < trusted`) when a deployment omits its own.
const DEFAULT_TRUST_CHAIN: [&str; 2] = ["suspicious", "trusted"];
/// Default resolver round-trip bound when a resolver omits `timeout_ms`.
const DEFAULT_TIMEOUT_MS: u64 = 30_000;
/// Inclusive bound on a resolver's `timeout_ms`.
...
pub enum AuthorityImpl {
    /// A compiled-in implementation: `approve` decides in-process, `hitl` elicits a human. A
    /// mandate's powers do not depend on which one backs it (`CFG-15`), so `approve` behind a
    /// covering mandate is an open gate the deployer chose — legitimate, and visible in review.
...
impl Config {
    /// Parse the spec-dialect TOML into a [`Config`]. Fails closed on any surface violation.
    ...
    /// The immutable, validated engine registry this policy builds.
...
// ---------------------------------------------------------------------------
// Raw serde mirrors of the TOML dialect. Every table is `deny_unknown_fields`,
// so a legacy `appa-proxy` key (`upstream_base_url`, `[[contracts.tool]]`,
// `output`, `rule`, `webhook`, `transformer`, `passthrough`) is a loud error,
// and every set mention must carry its operator (a bare list is a type error).
// ---------------------------------------------------------------------------
...
/// trust is the top rank (the user is the trust boundary), an omitted audience is public.
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct RawBoundary {
    trust: Option<String>,
...
/// `L0`: the most permissive established label — top trust, public audience.
fn default_boundary_label(chain: &TrustChain) -> Label {
    Label::new(Dim::Known(top_trust(chain)), Dim::Known(Audience::Public))
}

/// The most-trusted rank in the chain (the chain is validated non-empty before this runs).
...
struct RawTool {
    ...
    /// Top-level `effects` on a tool declares its `emits` (distinct from `requires.effects`).
    #[serde(default)]
    effects: Vec<String>,
    ...
    /// The tool's argument schema (JSON Schema as a TOML table), advertised to the model in the
    /// tool's wire schema. Advisory only — the engine checks flows, never argument shapes.
...
/// A delta or sanitizer-target audience: `{ exactly = [...] }`.
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct RawExactly {
    exactly: Vec<String>,
...
/// A concrete reader set. `["public"]` is the whole universe; any other list is a restricted set of
/// symbolic readers. `public` cannot be mixed with named readers, an argument placeholder (`$x`) is
/// not a reader here, and an empty set is rejected (nobody is not a set a delta/cap ever means).
...
/// An `includes` recipient: a concrete reader set, or a single `$arg` placeholder resolved from the
/// call's arguments at check time.
...
mod tests {
    ...
    /// The spec worked example (spec §"Worked example"), the same config `docs/contracts.md` documents.
    const WORKED: &str = r#"
version = 1
...
"#,
    ...
    fn legacy_proxy_dialect_is_rejected() {
        // Every stale `appa-proxy`-era shape is an unknown key under the new dialect.
        for legacy in [
    ...
    fn bare_operatorless_audience_list_is_rejected() {
        // A set mention without its operator is ambiguous — the dialect forbids it.
        assert!(matches!(
...
"#),
    ...
    fn builtin_approve_may_back_a_cover_bearing_mandate() {
        // `CFG-15`: a mandate's powers do not depend on the implementation behind them. Wiring
        // `approve` to a covering mandate is an open gate the deployer chose, legitimate per
        // `THR-3` and visible in review — not a load error.
        let cfg = config(
...
"#),
    ...
    fn builtin_hitl_may_back_a_cover_bearing_mandate() {
        // A cover ceiling is legitimate behind any implementation (`CFG-15`); the WORKED
        // fixture's approver vouches readers this way.
        let cfg = config(
            "version = 1\n[[authority]]\nname = \"a\"\n[authority.mandate]\ncan_raise_trust_to = \"trusted\"\n[authority.implementation]\nbuiltin = \"hitl\"\n",
        );
        assert_eq!(
    ...
    fn may_cast_audience_requires_an_operator() {
        // A bare reader list without `exactly` is operator-less — a type error, not a silent set.
        assert!(matches!(
            err(r#"version = 1
[[cast]]
name = "c"
...
"#),
        ...
        // With the operator it parses and carries the ceiling.
        let cfg = config(
            r#"
version = 1
[[cast]]
...
"#,
    ...
    fn algebraic_registry_lints_run_at_parse() {
        // An empty mandate is an engine load lint — a returned Config is always loadable, so parse
        // itself must reject it (not a later, separate build step).
        assert!(matches!(
            err(
                "version = 1\n[[authority]]\nname = \"noop\"\n[authority.mandate]\n[authority.implementation]\nbuiltin = \"approve\"\n"
            ),
...
"#),
    ...
    /// The configs `docs/contracts.md` documents must be real: every complete `version = 1` block
    /// there parses under this loader (binds the doc to the dialect it claims to teach).
    #[test]
