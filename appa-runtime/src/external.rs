...
//!
//! An authority never sees value or argument bytes: an [`AuthorityRequest`] names the call by
//! tool and canonical digest and carries the typed review context — the trajectory label fold at
//! review time and, per referenced argument Value, its label and provenance — plus the requirement
//! gaps. Never the tool result body or the non-recipient argument bytes (an email body a
//! downstream call would send). The one call-derived datum it carries is the *recipients of the
//! proposed release* (a `Gap::Includes`), because that is the subject the authority authorizes.
//! The context put to the authority is persisted verbatim in the `Ruling` it produces
//! (`AuthorityReview`), so the log replays the review itself. All HTTP backends use the runtime's
//! redirect-disabled [`HttpClient`], and read the resolver's answer through a size-capped stream.
...
impl BuiltinAuthority {
    /// Parse a config `builtin` name into the closed set (unknown names are rejected at load).
...
/// How an authority's rulings are produced.
#[derive(Clone, Debug)]
...
/// The input to a sanitizer derivation: the confined body it may relabel (audience-only).
#[derive(Clone, Debug, Serialize)]
pub struct SanitizerInput {
    pub body: String,
}

/// The result of a sanitizer derivation. `Failed` fails closed — no value is admitted.
...
/// The input to a cast resolution: the value's confined body and the dimension left Unknown.
#[derive(Clone, Debug, Serialize)]
...
/// A resolver-implemented cast (a constant cast is engine-side and needs no backend here).
#[derive(Clone, Debug)]
...
/// The wire answer: exactly one dimension, `{"trust": "rank"}` or `{"audience": ["reader", ...]}`.
#[derive(Debug, Deserialize)]
struct CastWire {
    trust: Option<String>,
...
/// The most a resolver answer may be. Answers are small JSON verdicts; a larger body is a misbehaving
/// or hostile resolver and fails closed, so a resolver cannot exhaust runtime memory.
const RESOLVER_BODY_CAP: usize = 64 * 1024;

/// POST `payload` as JSON and deserialize a 2xx response body into `T`. Any transport fault, non-2xx
/// status, oversized body, or malformed body yields `None` — the caller's fail-closed default. The
/// response streams through a `cap`-bounded reader, so it never buffers unboundedly.
...
mod tests {
    ...
    /// A call whose arguments embed a secret and whose one argument reference names the admitted
    /// store value below — used to prove neither byte stream reaches the wire.
    ...
    /// A branch holding one admitted tool-result value (id 0) whose body is a second secret: the
    /// authority may see its label and provenance, never its bytes.
    ...
    /// A one-shot server that reads the whole request (headers + the Content-Length body) and replies
    /// with a canned status + JSON body, returning what it received.
    ...
    /// Read an HTTP request completely: headers, then exactly the Content-Length body bytes.
    ...
    async fn http_resolver_oversized_response_fails_closed() {
        // A 2xx body past the 64 KiB resolver cap is refused rather than buffered — the resolver
        // cannot exhaust memory, and the answer fails closed (Abstain).
    ...
    async fn http_authority_request_shows_context_but_no_bytes() {
        // A realistic block: send_email(body: <secret>, to: auditor) with an unmet `includes` on the
        // resolved recipient — the exact release an authority would be asked to clear. The one
        // argument reference names an admitted store value whose body is a second secret.
        let projection = seeded_projection();
        ...
        // Neither the argument payload nor the referenced store value's bytes cross the wire...
        ...
        // ...while the typed review context does: identity, the trajectory fold, and the reference's
        // label + provenance.
        ...
        // The recipient of the proposed release reaches the authority — it is the subject the
    ...
    fn a_dangling_argument_reference_refuses_the_request() {
        // A reference to a value the branch does not hold never reaches an authority.
        let projection = seeded_projection();
        ...
        // A sibling branch's value is likewise refused — foreign, not reviewable here.
    ...
    fn redact_email_replaces_addresses_only() {
        ...
        // Whitespace and non-email tokens are preserved verbatim.
    ...
    fn cast_answer_parses_one_dimension_or_declines() {
        ...
        // Unknown rank, both dimensions, or neither → decline.
        assert_eq!(
            parse_cast_answer(
                CastWire {
    ...
    async fn http_cast_resolves_unknown_then_declines_on_fault() {
        ...
        // Nothing listening → unresolved (fail closed).
