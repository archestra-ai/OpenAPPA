...
//!
//! An authority never sees the payload: an [`AuthorityRequest`] names the call by tool + canonical
//! digest + argument value-ids and the requirement gaps — never the tool result body or the
//! non-recipient argument bytes (an email body a downstream call would send). The one call-derived
//! datum it carries is the *recipients of the proposed release* (a `Gap::Includes`), because that is
//! the subject the authority authorizes. All HTTP backends use the runtime's redirect-disabled
//! [`HttpClient`], and read the resolver's answer through a size-capped stream.
...
mod tests {
    ...
    /// A call whose arguments embed a secret value — used to prove the secret never reaches the wire.
    ...
    async fn http_authority_request_shows_recipients_but_not_the_payload() {
        ...
        // resolved recipient — the exact release an authority would be asked to clear.
        ...
        // The payload (the email body) never crosses the wire...
        ...
        // ...but the recipient of the proposed release does — it is what the authority authorizes.
