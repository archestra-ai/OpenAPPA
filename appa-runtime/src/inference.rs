...
//!
//! The runtime builds every request solely from server-held log facts (RP1/CC2) and never trusts the
//! model's output as history — a [`Completion`] is just the next move the turn-drive inspects: free
//! assistant text, tool-call proposals, or both. The client is non-streaming and fails closed: any
//! transport fault, non-2xx, oversized body, or malformed response is an [`InferenceError`], which
//! the drive turns into a typed terminal rather than trusting a partial answer.
//!
//! (Implementation note: the plan sketched `rig` for this client. We use the runtime's own
//! OpenAI-compatible wire structs plus the redirect-disabled [`HttpClient`] instead — the north face
//! already owns those structs, so this is a smaller, fully socket-testable surface with no extra
//! dependency. The upstream contract — POST an OpenAI chat-completions request with tools, read back
//! an assistant message with `tool_calls` — is unchanged.)
...
/// The largest inference response body the runtime will read (completions are small; a larger body
/// is a misbehaving endpoint and fails closed).
const COMPLETION_BODY_CAP: usize = 4 * 1024 * 1024;

/// One model move the turn-drive inspects: optional free text plus any proposed tool calls.
...
impl Completion {
    /// Does the model want to call tools this round (versus yield a final answer)?
...
/// A configured upstream model endpoint.
#[derive(Clone, Debug)]
...
mod tests {
    ...
    /// A one-shot OpenAI-shaped completion server that returns `response_body` and captures the
    /// request it received.
