...
//!
//! [`Mediator`] owns the policy, pure engine, session store, and every south backend. Inference and
//! turn orchestration belong to the embedding harness.
...
/// Why a mediator could not be assembled from an already validated config.
...
/// input (`POS-6`) and never policy (`CFG-18`): the head instructs the model, while the policy
...
/// A newly seeded child whose turn lease was reserved before the child became visible.
...
impl Mediator {
    /// Assemble with builtin fixtures for policy tools that have no configured HTTP implementation.
    ...
    /// Advertise policy tools in name order, followed by the role-specific reserved surface.
    pub fn advertised_tools(&self, is_child: bool, can_fork: bool) -> Vec<WireTool> {
        let mut tools: Vec<WireTool> = self
            ...
            .collect();
        // The remedy tool always names what an acceptance costs; it names the fork alternative only
        // where fork is itself advertised, so no session is pointed at a tool it cannot see.
    ...
    /// Fork and seed a child in one store critical section. A failed seed leaves no child session.
    ...
    /// Seed a child and reserve its turn lease before publishing its session id.
    ...
    /// Fork from the north/host boundary only after the parent's active turn completes.
    ...
    /// Read-only family facts for audit and replay. State mutation remains inside the mediator.
...
/// The remedy tool's schema. The acceptance cost is unconditional — it is a fact about narrowing;
/// the escape it names depends on what this session can actually do.
...
mod tests {
    ...
    fn a_transcript_head_admits_only_content_bearing_system_and_developer_messages() {
        ...
        // A user turn is trajectory content, never the head the host pins.
        assert!(matches!(
            TranscriptHead::new(vec![WireMessage::user("hi")]),
            Err(InitError::BadTranscriptHead(role)) if role == "user"
        ));

        // Content-only: a head message carrying a tool call or a tool-call id is refused, which is
        // the shape the retired policy parser could not express in the first place.
