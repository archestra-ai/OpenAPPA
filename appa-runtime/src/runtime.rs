...
//! [`Config`].
//!
//! Assembly is where the config's *descriptors* become live *backends* (a closed enum each — no
//! `dyn`): every registered authority/sanitizer/resolver-cast maps to its backend, and every
//! registered tool must resolve to an execution backend — an `http` impl from the policy, or a
//! programmatically-supplied `builtin` (the test/self-contained fixtures). A registered tool with no
//! backend is a load error, not a runtime surprise (the S12 coverage check). Two server-owned reserved
//! tools are pinned here — `execute_remedy_plan` for every session, `submit_result` for child sessions
//! — and may never collide with a policy tool or be injected via the north request (RP1/RP6).
...
/// The reserved tool a child session additionally pins: return one value to the parent (RP6).
pub const SUBMIT_RESULT: &str = "submit_result";

/// Why the runtime could not be assembled from a (already valid) config.
...
/// Per-turn resource ceilings (RP2). Any exhaustion ends the turn in a typed policy-stop terminal.
...
pub struct Budgets {
    ...
    /// The largest tool-result body admitted as a value (RP3); a larger 2xx body seals.
...
/// The assembled server. Immutable after `new`; the drive borrows it and appends to the store.
pub struct Runtime {
    config: Config,
    engine: Engine,
    store: SessionStore,
