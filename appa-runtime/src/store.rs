...
/// One family's shared log and version. Guarded as a unit so the CAS check and the append are one
...
/// One session's fixed metadata: its owner, its family (shared log), its immutable parent link, and a
/// per-trajectory **turn lease** — held across a whole `drive_turn` so at most one turn runs per
/// session at a time. This serializes the append of a session's assistant round and its per-call
/// terminal responses, which the transcript's positional pairing (RP1) depends on; sibling branches
/// (distinct trajectories) run concurrently under their own leases and only meet at the shared,
/// CAS-guarded family log.
#[derive(Clone, Debug)]
...
/// The in-memory store of all sessions and their family logs.
...
impl SessionStore {
    ...
    /// Mint a fresh root session (its own family) bound to `tenant`.
    ...
    /// The session's fixed parent link (`None` for a root). Bound at fork, never reparented.
    ...
    /// The family log and current revision, for reprojection. The caller must own `session`.
...
/// Resolve a session for a caller, refusing an unknown id (never leaking existence) or one owned by
/// another tenant — the authenticated-isolation check RP1 requires.
fn require_owned<'a>(
...
mod tests {
    ...
    /// A seed builder standing in for the engine's `seed_child`: a `Fork` boundary at the parent.
    ...
    fn concurrent_double_consume_is_rejected() {
        ...
        // Two batches computed against the same basis: the second is a stale double-consume.
        store
            .conditional_append(&tenant, &s, batch(Revision::ZERO, vec![boundary(&s)]))
            .unwrap();
    ...
    fn finalize_lands_in_bounded_steps_under_contention() {
        ...
        // A competitor advances the family revision continuously via CAS-retry — genuine, overlapping
        // contention that finalize must cut through in bounded steps.
        let competitor = {
        ...
        // Finalize appends the close at whatever revision it finds — one lock acquisition, no loop.
        let final_rev = store
        ...
        let (_, rev) = store.snapshot(&tenant, &s).unwrap();
        // Every competitor append plus the single finalize — no lost update, no double-append.
    ...
    fn finalize_is_idempotent_when_the_close_is_already_in_the_log() {
        ...
        // `decide` reprojects the *shared log* and appends the close only if it is not already there
        // — the real double-close guard, keyed off log state (not an out-of-band flag). Two
        // finalizers, each reprojecting, must commit exactly one close.
    ...
    fn fork_commits_the_seed_atomically_into_the_shared_family_log() {
        ...
        // The child's very first fact is its Fork seed — visible in the shared family log (RP6),
        // so projection never treats it as a fresh-slate root.
    ...
    fn a_rejected_seed_registers_no_child() {
        ...
        // No child was registered and the family log is untouched.
