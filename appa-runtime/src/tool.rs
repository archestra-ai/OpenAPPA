...
//! about the world — effects commit), **failed** (nothing committed), or is **indeterminate** (a
//! timeout/transport fault — it may or may not have run, so nothing commits and the model sees a
//! sealed token). Crucially, a failure's backend bytes are **never** forwarded: [`ToolOutcome::Failure`]
//! carries no payload, and a 2xx whose body exceeds the cap is a [`BodyDisposition::RejectedTooLarge`]
//! success — effects commit (the tool reported success) but no value is admitted (never
//! truncated/attacker-controlled bytes). Backends are a closed set (`builtin` fixtures, `http`), so
//! dispatch is a plain enum match — no `dyn`, no trait object.
...
/// Whether a success carried an admissible body or one too large to admit.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum BodyDisposition {
    /// A body within the cap — admitted as the tool result value.
    Available(String),
...
pub enum ToolOutcome {
    /// The tool reported success. Effects commit; the body is admitted or sealed per its disposition.
    Success { body: BodyDisposition },
    /// The tool reported failure (non-2xx). No effects, no value — the model sees a sealed token.
    Failure,
    /// The invocation's fate is unknown (timeout/transport fault). Treated like failure downstream —
    /// no effects, no value — but distinct in audit: the tool *may* have run.
    Indeterminate,
}
...
pub enum BuiltinTool {
    /// Succeeds with a fixed body.
    Echo(String),
    /// Succeeds with a body larger than the cap (the oversized-2xx fixture).
    Oversized(usize),
    /// Reports failure.
    Fail,
...
/// An HTTP tool backend: POSTs `{tool, arguments}` to a URL and classifies the response.
#[derive(Clone, Debug)]
pub struct HttpTool {
    url: String,
    timeout: Duration,
...
impl HttpTool {
    ...
    async fn invoke(&self, call: &RenderedCall, cap: usize) -> ToolOutcome {
        ...
        // Classify on the status *before* reading any bytes: a non-2xx never buffers its body, so a
        // backend cannot exhaust memory behind an error and its bytes never travel.
...
/// Stream a 2xx body, buffering **at most `cap + 1` bytes**: copy only up to the one-byte-over-cap
/// mark from each chunk, then seal the moment the buffer exceeds the cap (effects still commit, no
/// value admitted). A transport fault mid-body is indeterminate. The `cap + 1`-th byte is what
/// distinguishes an exactly-`cap` admissible body from an oversized one — a backend cannot make the
/// runtime allocate more than that.
...
/// A registered tool's execution backend — the closed set of ways the runtime runs a tool south.
#[derive(Clone, Debug)]
...
impl ToolBackend {
    /// Invoke the tool and classify the result. `cap` bounds an admissible body.
...
mod tests {
    ...
    fn non_2xx_is_failure_and_discards_the_body() {
        // The failure carries no payload — the backend's error bytes cannot travel to the model.
        assert_eq!(
    ...
    fn body_exactly_at_cap_is_admitted_one_over_is_sealed() {
        // The boundary: `cap` bytes admit, `cap + 1` seal.
        assert_eq!(
    ...
    async fn http_tool_admits_a_body_exactly_at_cap_over_the_wire() {
        ...
        // Streaming read admits exactly `cap` bytes.
        assert_eq!(
    ...
    async fn http_tool_seals_at_one_byte_over_cap_over_the_wire() {
        // The boundary through the streaming reader: `cap + 1` served bytes seal.
    ...
    async fn http_tool_posts_rendered_call_across_a_real_socket() {
        ...
        // Parse the captured request structurally: the wire carried exactly the rendered call.
    ...
    async fn http_tool_oversized_2xx_seals_across_the_wire() {
        ...
        // A cap below the served body → sealed; effects still commit.
        assert_eq!(
    ...
    async fn http_tool_treats_connection_refused_as_indeterminate() {
        // Nothing is listening on this port — a connection fault is indeterminate, not failure.
    ...
    /// A one-shot server that captures the **full** request (headers, then exactly the
    /// Content-Length body bytes — robust to a TCP split after the headers) and replies with a
    /// canned status + body.
