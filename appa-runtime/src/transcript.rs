...
//! surfaced otherwise (`BlockFeedback`). The two are paired to their calls **positionally**: the
//! turn-drive (RP2) processes calls serially and appends exactly one terminal response fact per call in
...
pub fn model_transcript(preamble: &[WireMessage], log: &[Fact], trajectory: &TrajectoryId) -> Vec<WireMessage> {
    ...
    // Child-return messages that arrived while a tool-call round is still awaiting its responses. A
    // sibling child (its own turn lease) can merge into the parent between the parent's assistant
    // tool-call message and the tool responses; emitting the child value there would produce
    // `assistant(tool_calls) → user → tool(response)`, which the Chat Completions contract forbids. So
    // hold such values and flush them once the round's responses drain.
    ...
    for fact in log.iter().enumerate().filter_map(|(index, fact)| {
        ...
        match fact {
            ...
            // A value a child branch returned and a merge absorbed into this (parent) branch — the
            // parent model must see the returned body to use it. Not a tool response: it pairs with no
            // proposed call, so it never consumes a pending slot, and it is deferred past any pending
            // tool responses to keep the tool-call/result sequence valid.
            Fact::ValueAdmitted {
                value,
            ...
            // The sealed response for a blocked/withheld/failed call. The fact names the call it
            // answers, so render with that explicit id and consume the matching pending slot (the two
            // agree under the per-session turn lease that serializes a turn's responses).
...
/// The target's root-to-leaf lineage, paired with each member's exclusive snapshot end.
...
/// Return the exclusive end of the last complete model-visible prefix before `limit`.
...
/// Once a tool-call round's responses have all landed, release any child returns held during it.
...
/// Render one recorded assistant round. A round with proposed calls becomes an `assistant` message
/// carrying those tool calls (and any free content); a round with only free text becomes a plain
/// `assistant` message; a round with neither renders nothing.
...
mod tests {
    ...
    fn pairs_a_tool_call_with_its_admitted_result() {
        ...
        // user, assistant(tool_calls), tool(call_1), assistant(final)
        assert_eq!(out.len(), 4);
    ...
    fn blocked_and_available_responses_pair_by_position() {
        // Two calls in one round: the first blocked (feedback), the second available (result).
        let log = vec![
            user("do two things"),
            Fact::AssistantMessage {
                trajectory: traj(),
        ...
        let out = model_transcript(&[], &log, &traj());
        // user, assistant(2 calls), tool(call_1=feedback), tool(call_2=result)
        assert_eq!(out.len(), 4);
        assert_eq!(out[2].tool_call_id.as_deref(), Some("call_1"));
    ...
    fn a_child_return_is_deferred_past_pending_tool_responses() {
        ...
        // A child merged a value into the parent *between* the assistant tool-call and its response.
        let log = vec![
            user("investigate"),
            Fact::AssistantMessage {
                trajectory: traj(),
        ...
        let out = model_transcript(&[], &log, &traj());
        // The tool response follows its assistant message directly; the child return is held until after.
