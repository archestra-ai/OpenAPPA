...
//!
//! A [`Turn`] owns one trajectory's lease from user admission through its terminal boundary. The
//! embedding agent owns inference and serial fork-join orchestration; this module owns every policy
//! check, south invocation, external decision, result admission, and lifecycle transition.
...
/// One provider-neutral assistant completion.
...
/// Shared ceilings for one root run and all of its descendants.
...
/// A requested charge could not fit in the shared run budget.
#[derive(Clone, Copy, Debug, Error, PartialEq, Eq)]
#[error("the shared run budget is exhausted")]
pub struct BudgetExhausted;

/// The one mutable budget shared by a root turn and all serially joined descendants.
#[derive(Debug)]
...
impl RunBudget {
    ...
    /// Charge one upstream inference before starting it.
    ...
    /// Charge one child creation after a [`Step::Fork`] is returned.
    ...
    /// Whether a turn at `parent_depth` may create a child under the depth ceiling.
    ...
    /// Whether any shared resource class has reached its ceiling.
    ...
    /// Remaining whole-run time, for bounding the next provider inference.
...
/// Why a turn could not begin. Cancellation while waiting admits no facts.
#[derive(Debug, Error)]
...
/// Why a turn operation could not complete.
#[derive(Debug, Error)]
pub enum TurnError {
    #[error("session store fault: {0}")]
    Store(#[from] StoreError),
...
/// Why the embedding agent stopped inference while no call was open.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum StopReason {
    InferenceFailure,
    BudgetExhausted,
...
/// One closed transition returned to the embedding agent.
#[derive(Debug)]
pub enum Step {
    Continue,
    Fork(ForkRequest),
...
/// A move-only, turn-bound request to run one child task.
#[derive(Debug)]
...
/// acceptance. The confined body remains runtime-only until that exact narrowing is accepted.
struct PendingCast {
    handle: String,
    dispatch: DispatchId,
    call: ResolvedCall,
...
impl Mediator {
    ...
    /// Admit a child through the lease reserved atomically with its fork.
...
impl Turn {
    ...
    /// The role-correct tool surface for this turn and the current shared fork budget.
    ...
    /// Rebuild model context solely from the runtime log and the host's transcript head.
    ...
    /// Record and mediate one provider completion. Ordinary calls execute serially.
    ...
    async fn mediate_call(
        ...
        // One resolution pass per proposal: once no cast lands, the residual block is final for
        // this loop — re-attempting on an unchanged state would spin.
        ...
        loop {
            ...
            match self.mediator.engine().check(&views, &call) {
                ...
                Ok(CheckOutcome::Block(raw)) => {
                    // `CHK-16`: attempt the registered casts on the unestablished values and
                    // re-check before surfacing an outcome — resolution is automatic, never an
                    // agent choice. What the agent eventually hears is the residual.
                    ...
                    let feedback = if !has_offers {
                        ...
                        // every advertised alternative in one cohort remains independently usable.
                        ...
                        *attempts += 1;
                        ...
                        // Handles are for engine-side plans only: an id-less redispatch names a
                        // call the agent makes for itself (`RMD-2`), so there is nothing to execute.
                        let offers = planned
                            .plans
                            .iter()
    ...
    async fn handle_execute_remedy(
        ...
        // Preflight, before any gate and before any authority hears of this: while a consumed
        // dimension stays unestablished, no ruling or acceptance may land (`CHK-16` — a fact
        // clears it, not a plan). The offers stay put, gated on the named facts; precedence over
        // the same-round gate so the agent hears the real reason.
        ...
        // Refuse stale offers before asking an authority. Execution validates again under the
        // append revision, but the external review itself must never be spent on obsolete state.
    ...
    async fn handle_submit_result(
        ...
        if let Some(parent) = self.mediator.store().parent_of(&self.tenant, &self.session)? {
            ...
            // `BRN-14`: a return resolves before it merges, whatever its policy — the split
            // below chooses how content crosses, while a label fact is established the same way
            // on both paths. Attempt the registered casts on the fold's unestablished values;
            // a residual blocks the merge with the values named and no plans offered.
            loop {
                let (log, revision) = self.mediator.store().snapshot(&self.tenant, &self.session)?;
                let projection = Projection::build(&log, revision);
                let facts = self
    ...
    fn check_raw_return(&mut self, call_id: &ToolCallId, body: &str) -> Result<RawReturnGo, TurnError> {
        ...
        match self.mediator.engine().check_child_return(&views, &self.session) {
            ...
            Ok(ReturnCheck::Block(ReturnBlock::Unestablished(facts))) => {
                // The pre-split resolution pass already ran (`BRN-14`), so this residual means no
                // registered cast could establish the fold: name the values, offer nothing.
    ...
    /// Attempt the registered casts on **every** unestablished fact (`CHK-16`): casts in
    /// registration order per value, the first resolution that establishes the dimension
    /// standing, each re-validated against the cast's declaration at admission (`SAN-8`).
    /// `Ok(true)` when at least one fact landed — the caller re-checks the fresh state —
    /// `Ok(false)` when nothing did, so the residual block is what the agent hears.
    async fn resolve_unknown(
        ...
        for target in facts {
            // Unconditional checkpoint: a pass whose casts produce no candidate would otherwise
            // never consult cancellation (constants that miss the dimension await nothing).
            if self.cancel.is_cancelled() {
            ...
            for cast in &self.mediator.config().registry_config().casts {
                ...
                // Admit under the family lock: siblings append to the same family revision, so a
                // CAS append could go stale forever under contention. The closure derives from
                // the live facts, so only a real append counts as landed — the callers' loops
                // then iterate on a strictly shrinking fact set.
    ...
    /// The surface for an offer that carries no branch alternative: a pending cast's acceptance and
    /// an authority's denial re-listing both name only the acceptance's reach (session or branch),
    /// and the fork budget gates the branch alternative alone — so it does not enter here.
