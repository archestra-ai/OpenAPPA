...
pub struct ChatCompletionRequest {
    /// The model id. Ignored on the north face (the server pins its own); set upstream.
    #[serde(default)]
    pub model: String,
    pub messages: Vec<WireMessage>,
    /// Tool schemas. On the north face a present value is **rejected** — tools are server-owned.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tools: Option<Vec<WireTool>>,
    /// Streaming is unsupported; a `true` value is rejected on the north face.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct WireMessage {
    ...
    /// Present on a `role: "tool"` result, naming the call it answers.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl WireMessage {
    ...
    /// An assistant message that proposes tool calls (no free content).
    ...
    /// A tool-result message answering `tool_call_id` — used **upstream only** (never accepted north).
...
/// The closed set of roles the runtime distinguishes.
...
/// A function invocation: a name plus a JSON **string** of arguments (the OpenAI convention).
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
/// A tool schema advertised to the model. Server-owned; never accepted from the north client.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct WireTool {
    #[serde(rename = "type", default = "function_type")]
    pub kind: String,
...
pub struct WireToolSchema {
    ...
    /// A JSON Schema object for the parameters — opaque to the engine.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl ChatCompletionResponse {
    /// A single-choice response carrying `message` with the given finish reason.
