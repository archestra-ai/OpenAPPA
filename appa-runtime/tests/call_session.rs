//! The per-call facade (`CallSession`) driven as a framework hook would drive it: check each call,
//! execute allowed ones out-of-band, report the outcome. No transcript is built by the SDK.
...
async fn an_allowed_call_is_checked_executed_and_reported() {
    ...
    // The hook would execute the tool here; we report the outcome.
    ...
    session.end_turn().unwrap();
    // The session is reusable for a fresh run.
...
async fn the_injection_ladder_blocks_the_email_when_the_authority_denies() {
    ...
    // The exfiltration: blocked on the trust floor; the remedy is declined by the authority.
...
async fn the_in_flight_guard_refuses_a_second_check_before_report() {
    ...
    // A second check while one is outstanding is refused.
    assert!(matches!(
        session.check_call(call("lookup", serde_json::json!({}))),
        Err(CallError::CallOutstanding)
    ));
    // end_turn is refused while a call is outstanding.
...
async fn the_run_lease_refuses_a_second_concurrent_turn() {
    ...
    // A second begin_turn before the first ends is refused (one run per session).
    assert!(matches!(session.begin_turn("second"), Err(CallError::TurnActive)));
    // check_call before a turn is refused.
    session.end_turn().unwrap();
    assert!(matches!(
        session.check_call(call("lookup", serde_json::json!({}))),
...
async fn an_unestablished_block_names_its_values_and_gates_offers_through_the_facade() {
    // The SDK profile registers no casts (they are refused at open), so an Unknown admitted by an
    // unannotated tool has nothing to attempt: the block goes straight to feedback with the
    // values named per UNK-3 — never the blanket verdict — and the mixed block's attention offer
    // stays gated on the missing fact when executed.
    let policy = r#"
version = 1
trust_chain = ["suspicious", "internal"]
[[tool]]
