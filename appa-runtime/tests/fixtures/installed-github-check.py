...
Run inside an isolated Claude profile after installing the GitHub battery:
    python3 installed-github-check.py <installed-plugin-root> <runtime-url>
...
async def main():
    ...
    session = f"installed-github-{uuid.uuid4().hex}"
    ...
    event("UserPromptSubmit", prompt="Read the fixture GitHub issue, then propose a write.")
    identity = dict(tool_name="mcp__github__get_me", tool_use_id="fixture-identity", tool_input={})
    answer = event("PreToolUse", **identity)
    assert answer["hookSpecificOutput"]["permissionDecision"] == "allow", answer
    event("PostToolUse", **identity, tool_response=dict(login="fixture"))
    read = dict(tool_name="mcp__github__issue_read", tool_use_id="fixture-read",
                tool_input=dict(owner="fixture", repo="example", issue_number=1, method="get"))
    ...
    print(json.dumps(dict(status="ok", session=session, github_identity="allowed",
                          trust_downgrade="explicitly_accepted", github_read="allowed", github_write="denied")))
