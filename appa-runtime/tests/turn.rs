...
/// The handle the latest feedback on `call_id` offers for the plan whose menu entry contains
/// `description`.
///
/// Handles are minted per session family and the return menu is ordered by what each plan costs
/// the parent, so neither the number nor the position is a fixture a test should pin. A test that
/// means "the raw acceptance" says so and lets the runtime name it.
fn offered_plan(
    mediator: &Mediator,
    tenant: &TenantId,
...
"#,
    ...
    // The void return stays on the menu — it is the right move when the parent needs no value —
    // but it no longer outranks a crossing that costs the parent nothing.
...
async fn return_acceptance_is_round_gated_but_a_residual_free_sanitize_is_not() {
    // A child return's Accept plan (and Sanitize with a residual) carries an acceptance and is
    // informed — next round only; a residual-free Sanitize accepts nothing and may execute in the
    // offering round.
    let mediated = mediator(
        r#"
version = 1
...
"#,
    ...
    // Child A: the raw-accept plan named in the same response as the submit is refused; the same
    // offer crosses in the next round. Handles run cheapest-first, so the residual-free sanitize
    // takes the lower one and raw acceptance the higher — see
    // `return_offers_run_cheapest_first`, which pins that ordering.
    ...
    // Child B: the residual-free sanitize plan executes in the round that offered it.
...
"#,
    ...
    // Neither fork ran a child here — the turn-binding is what is under test, so the join reports
    // a trajectory that never returned.
...
async fn child_context_stops_at_the_fork_and_raw_returns_cross_once() {
    ...
    // A value return ends the errand: the returned child cannot be re-driven — refused before any
    // fact is admitted, so the family log and revision stay untouched.
    ...
    // Nor can it become a fork parent, through either fork entry point.
...
/// The join tells the parent one thing: did a value cross. A void return and an abandoned errand
/// are the same event from the parent's side — spec "Branching", the void carries zero bits of
/// child-derived content — while a crossing is distinguishable because it is already visible.
/// Asserted as equalities between runtime outputs, so it survives any rewording of either message.
#[tokio::test]
...
async fn a_residual_bearing_sanitize_return_plan_is_round_gated() {
    // The sanitizer clears the audience narrowing but a trust narrowing survives (audience is the
    // only sanitizer territory), so the offered plan carries an accepted residual — informed like
    // any acceptance: refused in the offering round, executed in the next.
    let mediated = mediator(
        r#"
version = 1
trust_chain = ["suspicious", "trusted"]
...
"#,
    ...
    let log = facts(&mediated, &tenant, &parent);
    // The sanitized value crossed with the residual trust narrowing accepted alongside.
    assert_eq!(
        log.iter()
...
async fn a_voided_child_is_re_drivable_and_may_still_cross_its_one_value() {
    // At-most-once binds value crossings, not endings: a void return writes no ChildReturn, so
    // the child begins a later turn and crosses its one value then — unlike a value-returned
    // child, which is closed.
...
"#,
    ...
    sanitized_turn
        ...
        .await
        .unwrap();
    // Resolved by kind: the return menu runs cheapest-first, so neither plan's number is fixed.
...
/// A denying authority that keeps serving and counts how many times it was consulted. The count is
/// what distinguishes `RMD-6` from the behaviour it replaced: a denial that only retires its own
/// offer leaves a sibling naming the same authority live, and executing that sibling consults the
/// denier a second time.
...
/// The JSON payload line of the latest `BlockFeedback` on `call_id` — the wire the model reads.
...
async fn a_landed_cast_clears_the_check_and_the_call_dispatches() {
    // `scan` is unannotated, so its result admits at Unknown in both dimensions. `send`'s trust
    // floor consumes the Unknown; the registered constant establishes it at check time (CHK-16),
    // automatically, and the re-checked call dispatches — no block ever reaches the model.
    let policy = r#"
version = 1
trust_chain = ["suspicious", "trusted"]
...
async fn every_unestablished_fact_is_attempted_and_the_residual_is_named() {
    // Two unannotated reads leave two Unknown values. `send` consumes both dimensions; the one
    // registered cast establishes trust on *each* value (all facts are attempted, not just the
    // first), and the audience residual blocks with both values named per UNK-3 — never a
    // blanket verdict.
    let policy = r#"
version = 1
trust_chain = ["suspicious", "trusted"]
...
async fn a_declining_resolver_leaves_the_residual_named_not_blanket() {
    // The resolver abstains (EXT-1: a malformed answer is an abstention), so nothing lands and
    // the block names the value whose dimension stayed unestablished.
...
async fn an_offer_stays_gated_on_missing_facts_before_any_gate_or_authority() {
    // `restrict` narrows trust (a real narrowing once the cast establishes it) and demands
    // attention, while its audience requirement consumes an Unknown no cast resolves: the mixed
    // offer is minted, and a same-round execution is refused by the unestablished preflight —
    // before the informed-acceptance gate and before the authority hears anything. The payload
    // names the facts, nothing is accepted or ruled, and no dispatch opens.
...
async fn a_sanitizer_bound_return_resolves_its_fold_before_crossing() {
    // BRN-14 on the sanitized path: the child fold's Unknown resolves before the return-policy
    // split. With both constants registered the fold establishes and the bound sanitizer
    // crosses; without them the merge blocks with the values named and no plans offered.
    let resolved = r#"
version = 1
trust_chain = ["suspicious", "trusted"]
...
"#;
    ...
    // The abstention variant: no casts registered, so the fold cannot establish — the merge
    // blocks, the values are named, and nothing crosses.
    let unresolvable = r#"
version = 1
trust_chain = ["suspicious", "trusted"]
