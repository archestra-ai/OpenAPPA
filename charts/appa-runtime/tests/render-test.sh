...
must_contain 'Before cluster actions, call skills with command appa-guide'
must_contain 'with offset 1 and limit 0'
must_contain 'without calling a'
must_contain 'mutating tool or opening a confirmation card'
must_contain 'approving that proposal before attempting its mutation'
must_contain 'fetch its manifest with helm_get_release'
must_contain 'A read is not automatically public or trusted'
must_not_contain 'When detailed reference'
must_not_contain 'reads use delta = {}'
must_not_contain 'To execute any requested mutation'
