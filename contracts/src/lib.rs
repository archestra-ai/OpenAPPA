...
/// An approval loop runs up to a bounded number of rounds, so the per-ruling
/// timeout caps how long a session can stay parked on one flow.
...
/// The `webhook = { url = "…", timeout_ms = … }` inline table on an
/// escalate authority.
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct WebhookSpec {
...
/// A declared transformer: a compiled-in builtin implementation bound to an
/// operator-declared precondition and output label. The `output` label is
/// the trust decision — the label every derivation wears, required and
/// total (a transformer with an unknown output is meaningless). TOML cannot
/// carry code, so `builtin` names one of the known implementations; phase 2
/// adds `webhook = {…}` as the out-of-process alternative.
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct TransformerSpec {
    name: String,
    builtin: String,
    /// Which source labels the transformer applies to; an omitted table or
...
mod tests {
    ...
    fn transformer_rejects_unknown_fields_everywhere() {
        for text in [
            // On the transformer itself (webhook is phase 2, not silently accepted).
            "[[transformer]]\nname = \"t\"\nbuiltin = \"redact-email\"\n\
             output = { trust = \"trusted\", audience = \"public\" }\n\
             webhook = { url = \"https://x.example\" }",
            // Inside precondition.
