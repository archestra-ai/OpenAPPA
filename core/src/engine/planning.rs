...
/// [`SimFlow`]: the argument tree and the checked flow the lifts scope to.
struct SearchCtx<'a> {
...
impl PolicyEngine {
    ...
    /// flow: a transformer application to any non-recipient leaf, in
    /// registration order.
    ...
    /// The deterministic authorize peels for one reduce-state: durable
    /// raises for the arg-borne confidentiality residual, then — when a
    /// residual remains — one candidate plan per clearing lift alternative
    /// (the least-privilege control-release variant and the plain lift are
    /// genuinely different asks; dominance, not enumeration order, decides
    /// between them).
    ...
    /// anchored ordinary solver misses), compose the final waiver (carrying
    /// its acknowledge-only facts), and keep the candidate iff every grant
    /// is authorizable and the projection then clears.
    ...
    /// dependency hides from the actual vector. The final waiver is shown
    /// the *actual* (unreleased) flow's residual, so the ruling authority
    /// still sees what the release itself will clear.
    fn joint_rescue(
        &self,
        base: &SimFlow,
    ...
    /// whose precondition no longer matches breaks the prediction);
    /// authorize steps apply their
        ...
        for step in steps {
            match step {
                ...
                PlannedRemedy::Authorize { authorization, .. } => {
                    ...
                    // The one shared coordinate→lift fold; a raise-only
                    // delta constitutes no lift (presence would clear
                    // acknowledge-only facts it never acknowledged).
...
/// longer waived — they route to Endorse steps — and acknowledge-only and
/// structural members contribute no lift.
fn needed_delta(violations: &[Violation]) -> Lift {
    ...
    for violation in violations {
        match violation {
            ...
            // Trust/audience route to Endorse; acknowledge-only and
            // structural members contribute no lift.
            Violation::Breach(
                Breach::TrustBelow { .. }
