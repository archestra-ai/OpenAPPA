...
/// Fails to compile when `$ty` implements `$bound`.
///
/// `Marker<()>` covers every type; `Marker<Forbidden>` covers only those
/// implementing `$bound`. Resolving `<$ty as Marker<_>>::PROOF` is
/// unambiguous only while `$ty` is outside the second impl.
...
/// Fails to compile when `$ty` stops implementing `$bound`.
