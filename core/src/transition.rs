...
impl ActionTransition {
    ...
    ///
    /// Stated over the tool and proposed effects rather than a pending action:
    /// the planner narrows a simulated flow one hop out, and the applier a live
    /// pending action, so the state is supplied as values and both hold the
    ...
    pub fn narrows(&self, tool: &ToolName, proposed_effects: &Effects) -> Result<(), TransitionFailure> {
        ...
        let narrower = match (proposed_effects.declared_set(), self.effects.declared_set()) {
            // Constraining an unknown-effect action to anything declared is
            // the sandbox case.
            (None, Some(_)) => true,
