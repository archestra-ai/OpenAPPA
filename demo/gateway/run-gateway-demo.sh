...
# In order: the environment, this checkout's repo-root .env, then the main
# checkout's .env (a linked worktree does not carry untracked files).
read_env_key() {
  [[ -f "$1" ]] || return 1
  local val
...
if [[ -z "${OPENROUTER_API_KEY:-}" ]]; then
  echo "no OPENROUTER_API_KEY: set it, or add it to the repo-root .env" >&2
  exit 1
fi
