...
#
# Prereqs: docker, kind, helm, kubectl, and OPENROUTER_API_KEY (or the repo-root .env).
set -euo pipefail
cd "$(dirname "$0")"
...
# Source OPENROUTER_API_KEY from the repo-root .env if not already set.
if [[ -z "${OPENROUTER_API_KEY:-}" && -f ../../.env ]]; then
  # shellcheck disable=SC1091
  set -a; . ../../.env; set +a
fi
: "${OPENROUTER_API_KEY:?set OPENROUTER_API_KEY (or put it in the repo-root .env)}"
