...
//!
//! A [`Session`] mediates coarsely: it cannot see which values the model
//! actually read, so every admitted value joins one running `context` set and
//! every model decision is treated as having read all of it. `L_flow`
//! therefore degrades to the fold of the whole visible context, while the
//! engine underneath stays value-granular.
//!
//! The session lives in memory only. The adapter keeps the source history and
//! reconstructs a fresh session from it by driving it in conversation order:
//! feed a user turn (with its label), feed an assistant turn's proposed
//! calls, feed a tool result, ask for a verdict.
...
/// Remedy budget per `pursue` call: acknowledge/waive chains are short;
/// anything needing more steps than this is not a flow the edge should
/// auto-clear.
const MAX_REMEDY_STEPS: usize = 8;

/// Resolver round-trips per verdict. Separate from [`MAX_REMEDY_STEPS`],
/// which resets on each `pursue` re-entry after an applied ruling.
...
pub enum Verdict {
    ...
    Permitted,
    /// Permitted after remediation; the trail names each applied remedy — an
    /// authority's grant, or a value derived by a registered transformer.
    /// When a derivation substituted the payload, `canonical_arguments` is
    /// the exact arguments the engine checked — the adapter must ship these,
    /// never the proposal's bytes.
    Granted {
        trail: String,
        canonical_arguments: Option<String>,
    },
    /// Dispatch path only: the canonical request was executed and its output
    /// recorded. `result` is the exact recorded body.
    Executed { output: ValueId, result: String },
    /// Dispatch path only: the executor failed; the action was closed with
    /// `record_failure` (committed effects stay committed).
    ExecutorFailed { reason: String },
    /// The policy proved the flow cannot proceed.
    Terminal {
        violations: Vec<Violation>,
        reason: BlockReason,
    },
    /// A remedy walk did not settle within its budget.
    Stalled {
        violations: Vec<Violation>,
        cause: StallCause,
    },
    /// The proposal itself was defective (stale, foreign, conflicting) —
    /// no state was touched.
    Refused(FlowRefusal),
    /// An external authority was required but no ruling was obtained; the
    /// flow fails closed by the absence of a grant.
...
/// Where a settled flow ends up before the permit is spent.
enum Settled {
...
pub struct Session {
    ...
    /// call id → the proposal, for driving a later result. Kept, not
    /// consumed: a duplicated result re-checks like the original.
    proposals: HashMap<String, Proposal>,
    /// Set across every await that holds a linear core capability, and
    /// permanently on any condemning error. A future dropped mid-await
    /// cannot clean up, so the flag stays set and every mutating call fails
    /// closed with [`EdgeError::Poisoned`].
    poisoned: bool,
}
...
impl Session {
    ...
    ///
    /// The re-check consults the resolver like any evaluation would: a grant
    /// that was check-scoped when the call was first permitted (an external
    /// authority's live ruling) is not stored state, so replaying the call
    /// needs the authority again. An adapter with no approval channel passes
    /// [`NoResolver`] and any escalating replay stays blocked.
    ///
    /// A result for an unknown call id or an uncontracted tool contributes no
    /// labeled context and is skipped: histories legitimately truncate (a
    /// call proposed outside the adapter's window has no proposer to bind
    /// to), and mediation covers contracted tools only — the same
    /// gradual-adoption stance every existing embedding takes. An adapter
    /// that wants stricter correlation enforces it before feeding the result.
    ///
    /// [`NoResolver`]: crate::resolver::NoResolver
    pub async fn past_tool_result(
        ...
        // Replay does not run the integrity guard: the dispatch already
        // happened, and a flow the guard would block never returned a permit
        // at verdict time, so it cannot legitimately appear in history.
        let built = self
        ...
        let blocked = match self.settle(built.request, resolver).await {
            ...
            // No ruling was obtained; the wording is the proxy's original
            // no-channel message, still accurate for its NoResolver case.
            ...
            // A refusal is a protocol/state defect, not a policy judgment:
            // the replayed history is malformed, so fail closed rather than
            // treat it as a permit.
        ...
        // A blocked replay condemns the whole session: the reconstruction is
        // wrong, not just this proposal. Clear what can be cleared and poison
        // so a retained session cannot keep advancing state.
        let _ = self.trajectory.abandon_pending();
        Err(self.condemn(blocked))
    }

    /// Decide the fate of one new proposed call — check only, the adapter's
    /// harness executes permitted calls itself. `proposer_body` is the exact
    /// bytes that proposed the call (the serialized wire call); it is
    /// admitted as the proposing assistant value, and the call is registered
    /// under its id so the executed result can later be fed back through
    /// [`Session::past_tool_result`] and taint the context. Always clears the
    /// pending slot, so sibling calls in one response evaluate independently.
    ///
    /// A `Permitted` verdict for a tool with no contract means the call is
    /// outside mediation entirely: nothing was admitted or registered, and
    /// its result will contribute no labeled context.
    pub async fn verdict(
        ...
        // Registered only once the arguments parsed: a malformed call never
        // runs, so no result of it should ever bind.
        self.proposals.insert(
            call.id.to_string(),
            Proposal {
                tool: tool.clone(),
    ...
    /// The executor sees the canonical tool identity — a remedy may have
    /// narrowed the action to a different tool than was requested. The
    /// executed output is recorded inline, so the call is not registered for
    /// a later [`Session::past_tool_result`].
    ///
    /// A `Permitted` verdict for a tool with no contract means the call is
    /// outside mediation entirely: `execute` was NOT invoked — the adapter
    /// runs the call itself, and its result contributes no labeled context.
    pub async fn dispatch(
        ...
        // Bookkeeping failures past this point condemn the session: the
        // token/receipt is consumed either way, and only the receipt could
        // have closed the released action.
    ...
    /// The trajectory's control-plane history, read-only.
    pub fn audit(&self) -> &[AuditEvent] {
        self.trajectory.audit()
    }

    /// Whether the pending-action slot is occupied.
    ...
    /// Poison the session and pass the condemning error through.
    fn condemn(&mut self, error: EdgeError) -> EdgeError {
        self.poisoned = true;
        error
    }

    /// Clear the pending slot after a check-only or blocked flow. Neither
    /// ever releases, so this cannot legitimately fail — if it does, the
    /// session is in a state this code did not produce: poison it.
    ...
    /// Run the flow to a permit or a blocked verdict, consulting the resolver
    /// for every external ruling the walk needs. The pending slot is left as
    /// the engine left it — callers clear or spend it.
    async fn settle(&mut self, request: ToolRequest, resolver: &impl AuthorityResolver) -> Settled {
        ...
        loop {
            ...
            // The await holds the linear `PendingApproval`: a dropped future
            // cannot clean up, so the flag poisons the session (see `guard`).
            self.poisoned = true;
            ...
            let ruling = match resolved {
                Ok(ruling) => ruling,
                Err(error) => {
                    // The edge never rules. No ruling is applied at all: the
                    // flow fails closed by the absence of a grant.
                    tracing::debug!(%authority, %error, "no ruling obtained; failing closed");
                    return Settled::Blocked(Verdict::Unresolved { authority });
                }
            };
            // `apply_approval` consumes the approval, applies the ruling, and
            // rechecks fail-closed. Continue from what it returns: a permit is
            // final (never re-pursue past a live token); only a still-
            // remediable flow goes another round.
    ...
    /// The remedies applied since `audit_from`, as decision-log reasons —
    /// what distinguishes a remediated permit from a clean one.
    ///
    ...
    /// The coarse request: the payload leaf is the call's arguments JSON,
    /// admitted as a value derived from the proposing assistant value (the
    /// fold is idempotent, so its label equals the proposer's — no decision
    /// changes; what changes is that a `DeriveValue` remedy now derives the
    /// *arguments*, not the assistant prose). Recipient strings likewise
    /// become their own derived values so the audience check can read them.
    /// Both are request-local: admitted to the trajectory, never inserted
    /// into `context`, so they do not enlarge later flows' control sets.
    /// Control is the whole context.
    ...
    /// The recipient-integrity guard: the payload textually contains any
    /// contract-designated recipient field, but the engine checks recipients
    /// as their own protected leaves — a derivation that rewrites the field
    /// inside the payload would dispatch a recipient the engine never
...
/// The synthetic argument field holding the call's arguments JSON.
const PAYLOAD_ARG: &str = "payload";

/// A built coarse request plus what the settled-verdict path needs to keep:
/// the payload leaf's identity (substitution detection) and the checked
/// recipient strings (the integrity guard's baseline).
struct BuiltRequest {
    request: ToolRequest,
    payload: ValueId,
...
/// Join violations into one human-readable summary line.
...
mod tests {
    ...
    /// The proxy's original test tool set: a tainting reader, a guarded
    /// mutator, and a tool with unknown requirements.
    const TOOLS: &str = r#"
        [[tool]]
        name = "get_logs"
    ...
    async fn tainted_replay_of_a_guarded_call_fails_closed() {
        ...
        // A blocked replay condemns the session: nothing may advance it.
        assert!(matches!(
    ...
    async fn external_authority_denial_is_terminal() {
        ...
        assert!(!s.pending_call());
        // The denial poisons nothing: a clean sibling call still permits.
        assert_eq!(check(&mut s, "delete_resource").await, Verdict::Permitted);
    }
    ...
    async fn no_ruling_fails_closed_without_fabricating_one() {
        ...
        // No ruling was applied: the audit trail holds no authorization.
    ...
    async fn a_dropped_in_flight_verdict_poisons_the_session() {
        ...
        let mut s = session(&format!("{TOOLS}{ESCALATE_AUTHORITY}"), "hi");
        {
            ...
            // Dropped here, mid-await, holding the pending approval.
        }
    ...
    async fn dispatched_output_taints_the_context() {
        ...
        // The recorded suspicious output must taint the next guarded flow.
        assert!(matches!(
    ...
    async fn executor_failure_closes_the_receipt() {
        ...
        assert!(!s.pending_call());
        // The action closed cleanly: the session keeps working.
        assert_eq!(check(&mut s, "delete_resource").await, Verdict::Permitted);
    }
    ...
    async fn a_checked_calls_result_taints_the_live_session() {
        // The verdict registers the proposal, so a live-session adapter can
        // feed the executed result back and its label reaches the context.
        let mut s = session(&format!("{TOOLS}{ALLOW_AUTHORITY}"), "hi");
        let verdict = s
            .verdict(
                "{}",
    ...
    async fn a_dropped_executor_future_poisons_the_session() {
        let mut s = session(TOOLS, "hi");
        {
            ...
            let mut cx = Context::from_waker(Waker::noop());
            assert!(matches!(fut.as_mut().poll(&mut cx), Poll::Pending));
            // Dropped here, mid-executor, holding the dispatch receipt.
        }
        assert!(matches!(
            s.past_tool_result("c1", "x", &NoResolver).await,
            Err(EdgeError::Poisoned)
    ...
    /// Two escalate authorities with split mandates: each grant resolves one
    /// violation, so the walk needs one resolver round per authority — the
    /// `apply_approval` → `Remediable` → re-`pursue` path.
    ...
    async fn an_externally_granted_result_feeds_back_through_the_resolver() {
        // A check-scoped external grant is not stored state: replaying the
        // executed call re-asks the authority. With the resolver present the
        // result lands and taints; the session is never condemned.
        let mut s = session(&format!("{TOOLS}{ESCALATE_AUTHORITY}"), "hi");
        let verdict = s.verdict("{}", call("mystery_tool"), &Rule(approve)).await.unwrap();
        assert!(matches!(verdict, Verdict::Granted { .. }), "got: {verdict:?}");
        s.past_tool_result("c1", "result", &Rule(approve)).await.unwrap();
        assert!(!s.pending_call());
        // Still healthy: a clean sibling call permits.
        assert_eq!(check(&mut s, "delete_resource").await, Verdict::Permitted);
    }
    ...
    /// The kagent-demo-shaped transformer policy: a private reader narrows
    /// the context audience, notify's sink wants the whole team back, the
    /// redactor's declared output widens the audience (declassification via
    /// a registered transformer — never a trust raise), and the only
    /// authority can release control but cannot touch audience or trust.
    ...
    /// Read the private source so the context audience narrows to
    /// `["operator"]` and the trust fold turns suspicious.
    ...
    async fn denied_release_after_derivation_stays_blocked_and_the_derived_value_remains() {
        ...
        // Facts only grow: the derivation applied before the denial and its
        // audit record legitimately remains.
        assert_eq!(applied_transitions(&s), 1);
    }

    #[tokio::test]
    async fn audience_widening_redactor_never_rescues_a_trust_violation() {
        // The delete fails on trust; the redactor's declared output changes
        // audience only, and no authority can raise trust — so the frontier
        // is empty (Terminal) and no derivation is ever applied. This is the
        // laundering-prevention property the demo's story relies on.
        let mut s = session(TRANSFORMER_POLICY, "why is checkout failing?");
        narrow_context(&mut s).await;
        let verdict = s
    ...
    async fn payload_and_recipient_values_are_request_local() {
        ...
        let _ = s
            ...
            .await
            .unwrap();
        // Only the proposing assistant value joins the context; the payload
        // value stays request-local so later flows' control sets (and any
        // control-release coordinates) do not grow with it.
    ...
    fn recipient_integrity_fails_closed_on_divergence_and_unreadable_canonicals() {
        ...
        // A tool without a recipients extraction has nothing to guard.
