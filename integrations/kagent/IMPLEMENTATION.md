...
| Human review on remedy plans | all | The elicitation channel needs an interactive MCP client the pod lacks. The designed route is `require_confirmation` on the reserved tool over kagent's stock approval flow, present on both runtimes — its standing as the authority's proof is an open decision, settled with the spec. Until then such plans stand unexecuted, and no answer grants nothing. |
