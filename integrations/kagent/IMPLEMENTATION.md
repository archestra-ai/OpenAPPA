...
- ModelConfig `appa-demo-model` over the Secret of the same name (`openai.apiKey` or `openai.existingSecret`). So the Models → Edit flow of the dashboard supplies the key after install. The model is `gpt-5.6-luna`.
...
| v0.9.12 | A-py | python · google-adk 1.31.1 | A2A, five cases in CI | informational, on the `run-e2e` label |
