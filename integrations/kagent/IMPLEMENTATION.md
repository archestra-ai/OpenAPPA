...
`appa-adapter-kagent` is the Rust adapter crate, a workspace crate compiled into `appa-runtime`. The runtime selects it through its closed `Adapter` enum ([appa-runtime/src/main.rs](../../appa-runtime/src/main.rs)). The plugins post the canonical hook envelope ([appa-runtime-api/src/wire.rs](../../appa-runtime-api/src/wire.rs)) to `POST /hook` directly; the crate derives the canonical tool id and whether a call is a spawn from the structured tool spelling the plugins send.
...
Both images read `APPA_RUNTIME_URL`, and both emit the same canonical wire.
...
| `after_tool_callback` | `ToolResult` | `DeliverValue{value}`: return a dict carrying the admitted value as it crossed. `ReplaceOutput{output}`: return a dict carrying the runtime's own words, spelled back into names the model dispatches. `Block`: return a withheld dict — the model reads `[appa] the tool result was withheld: <reason>`. A call the plugin already answered — a deny, a review, or a failure — is closed at that moment and reported once. The plugin remembers the call by its `function_call_id`, so a tool result that spells the deny marker itself cannot skip this gate. | `functions.py` 547-576 |
| `on_tool_error_callback` | `ToolResult` with `Failure` outcome | `Ack`: return None, and the original error propagates. `DeliverValue`, `ReplaceOutput` or `Block`: return a dict (the admitted value as it crossed on `DeliverValue`, the withheld text on `Block`). The dict becomes the function response the model reads. Does not fire for a `before_tool_callback` raise — a `Refuse` stays terminal. | `functions.py` 535-545 |
...
| `after_tool_callback` | `ToolResult` | `DeliverValue{value}`: the returned dict carries the admitted value as it crossed. `ReplaceOutput{output}`: the returned dict carries the runtime's own words, spelled back into names the model dispatches. `Block`: a withheld dict — the model reads `[appa] the tool result was withheld: <reason>`. A call the plugin already answered is closed by its `function_call_id` and reported once, so a tool result that spells the deny marker itself cannot skip this gate. | `functions.py` 652-656 |
| `on_tool_error_callback` | `ToolResult` with `Failure` outcome | `Ack`: return None, and the original error propagates. `DeliverValue`, `ReplaceOutput` or `Block`: return a dict (the admitted value as it crossed on `DeliverValue`, the withheld text on `Block`). The dict becomes the function response the model reads | `functions.py` 544-563, 595, 641 |
...
2. Fill `reasoning_effort` on the OpenAI model from `APPA_KAGENT_OPENAI_REASONING_EFFORT` when the rendered config leaves it unset. The v1alpha2 `ModelConfig` enum admits `minimal`, `low`, `medium` and `high`, and no `none`. The gpt-5.6 models this integration runs against (sol, terra) refuse function tools on chat completions without `reasoning_effort: "none"`. The Terra installation sets the env to `none` for `appa-guide` and every demo Agent. A value the CRD set wins, and a model of another type stays as it is. The go main applies the same env to `Model.ReasoningEffort`.
...
6. Append the runtime-owned toolset: a `McpToolset` over streamable HTTP at `$APPA_RUNTIME_URL/mcp`. It exposes remedy execution, deterministic state and battery matching, and vouched policy/battery management operations (see [Remedy-plan execution](#remedy-plan-execution)).
...
Both plugins classify a call by the spelling its inventory gives the tool (`inventory.py`, `inventory.go`): a remote agent of the rendered config spells `agent:<namespace>/<agent>`, and only that class is a spawn. A `transfer_to_agent` call names no inventory entry, so the Go plugin refuses it at `BeforeToolCallback` like any undeclared tool. The [VERIFICATION.md](appa-kagent-adk-go/VERIFICATION.md) file states this path in prose, and no test exercises it.
...
- **Tool naming and coverage.** Policies can use the native tool names the agent calls. Canonical names remain available; optional `server` qualifiers and deployment `server_aliases` restrict a rule to one source. The plugins discover MCP metadata through the host's authenticated sessions before the opening prompt and before exposing updated tools. Filters are optional. Each source is `server-<sha256(exact configured URL)>`, without hostname or provider inference. Uncovered MCP tools are excluded from exposure; uncovered builtins and delegations are individually refused before execution. They do not prevent covered actions in the same conversation. Ambiguous identities and malformed configuration still reject the initial inventory. Unreachable sources remain explicitly unknown. Covered late tools become callable under the pinned policy; invalid updates do not remove working tools. Discovery never executes a tool or grants permission. Calls still cross the runtime admission gate. Agent tools retain `agent/<namespace>/<agent-name>` identities; builtins and out-of-band gates retain `host/kagent/<name>` and `host/kagent-gate/<name>`. The reserved `execute_remedy_plan` needs no authored entry.
...
Each plugin emits one JSON event per gated callback, with the five exceptions the python section names. The first user message of a run emits two events: the session start or the child start, then the prompt. A child scope emits `child_end` at each stop of the child. Four moments emit none. Those are the reviewed first pass of the control call, the deny or review dict at after-tool, the Go error path at after-tool, and both Go error callbacks after a failed-closed call gate. Every event is the canonical envelope of [appa-runtime-api/src/wire.rs](../../appa-runtime-api/src/wire.rs): `protocol: 1`, `adapter: "kagent"`, the kind, the unprefixed trajectory ids (the runtime prepends `kagent:`), the structured tool spelling, the arguments, outcome, and value fields. The event asserts no spawn: the runtime derives the canonical tool id and whether the call is a spawn from the spelling ([Tool naming and coverage](#annotators)).

The wire carries no policy meaning. The arguments cross as the plugin serializes them from the parsed call ADK hands it (`json=` in python, `json.Marshal` in Go). The runtime passes that serialization through unparsed, and the Engine canonicalizes it. The reserved tool crosses as `appa:execute_remedy_plan`, so the runtime recognizes its own tool and binds the vouch.

Three fields carry what the plugin routes without the model. A `deny_call` carries `offers`: every remedy the block offers, in the order the feedback lists them. A plan that declares a return of a child also carries `returns`, either `"as_spoken"` or `{"sanitizer": …}`. The plugin reads that field to recognize a held spawn ([Delegation and the fork](#delegation-and-the-fork)). A `deny_call` carries `review` too: the offers whose plans consult a `hitl` authority, with the text the person reads. A `tool_call` of the reserved tool may carry `ruling`, the answer of the person ([Human review](#human-review)). The plugin obtained it through the kagent confirmation. The value is `approve` or `deny`. Any other spelling counts as malformed, and the runtime refuses the event. Every decision carries `protocol: 1`, and a plugin refuses an answer under another protocol. A decision that stands in for a result names its content: `deliver_value` carries the value the runtime admitted, which the plugin delivers as it crossed, and `replace_output` carries the runtime's own words, whose tool spellings the plugin spells back into the raw ADK names (`_for_model`, `forModel`).

The fixture rows `tool_call_control` and `tool_call_control_ruled` in [wire-events.jsonl](../../marketplace/plugins/kagent/fixtures/wire-events.jsonl) pin both shapes, and the rows `tool_call_mcp`, `tool_call_agent`, `tool_call_builtin` and `tool_call_gate` pin one spelling of each tool class. The rows `child_end_returned` and `child_end_void` pin the stop of a child, with a value and without one. The python plugin spells the reserved name once, `RESERVED_TOOL` in `wire.py`, and its wire spelling once, `CONTROL_TOOL`. The builtin manifest [kagent-builtins.json](../../marketplace/plugins/kagent/fixtures/kagent-builtins.json) pins, per lane and kagent version, the tools the stock runtime attaches itself; each plugin ships a byte-identical copy and a test holds the three together. One wire serves both runtime images.
...
- When the child opens, Python: `SessionIdentity` pins the lineage-derived root and child pair for the invocation. The plugin sends `child_start` only for a pair it has not opened. `remote_agents.py` gives each new call its own stock-tool copy and context ID, even within one turn. Approval resumes reuse the context ID in the confirmation payload. The original toolset owns HTTP client cleanup.
- When the child opens, Go: the plugin pins the lineage-derived pair and opens it once. `discoveryAgentConfig` copies the remote-agent declarations and enables `IsolateSessions` before SDK construction. The SDK generates a new context ID for each new call and retains the paused task's context ID on resume. This works on the stable CRD without exposing another user setting.
- Activity identity: both plugins return `subagent_session_id` with the actual child result. The Python wrapper omits the stock executor's per-turn pre-generated ID. The pinned UI can then use the response ID without a stale call-metadata value taking precedence; the activity link appears when a result arrives.
...
| Child activity before the first result | A-py, A-go | Isolated remote calls report their actual child ID with the result. The activity link becomes available then; no pre-generated ID points the operator at another child session. |
...
| A remote agent's identity is its declared name, not its endpoint | all | The inventory spells `remote_agents[i]` from its `name` and reads the `url` no further, so a config that names one agent and dispatches to another endpoint would carry the policy identity of the name. The kagent controller cannot render that pair: it fills both fields from the one Agent object a tool reference resolves to — the name from the object reference, the URL from `toolAgentURL` of the same object ([rc4 compiler.go#L381-L387](https://github.com/kagent-dev/kagent/blob/v0.10.0-rc4/go/core/internal/controller/translator/agent/compiler.go#L381-L387)) — and an Agent tool reference is a `TypedReference` (kind, name, namespace) with no URL field. Binding the name to the host would refuse two renderings the controller does emit: a global proxy rewrites the URL to the proxy host and carries the real one in `x-kagent-host`, and a sandbox agent is reached at the controller under `/api/a2a-sandboxes/<ns>/<name>`. A config authored by hand and mounted past the controller can still decouple them; no check runs. |
...
- The `demo-tools` Deployment, Service, and `RemoteMCPServer` ([demo/Dockerfile](demo/Dockerfile)). Two canned public-GitHub tools, `get_file_contents` and `issue_write`, use contracts from the shipped GitHub battery. The demo policy includes that battery and binds its `github` namespace to the demo MCP endpoint; the chart renders the binding for the deployment namespace.
- A standalone `appa-demo-mocks` Deployment and Service. They implement `runbook-readers`, `release-window`, `change-board`, and both deterministic sanitizers. Curl commands in [demo.appa.toml](demo/chart/files/demo.appa.toml) forward consult envelopes from the runtime to this Service. Direct cleartext URL bindings remain loopback-only.
- `ConfigMap/appa-kagent-demo-policy`, an inert rendered policy template. It is neither mounted nor served. `appa-guide` verifies the Helm release, compares the template with serving policy, and submits an approved complete merge through `appa_update_policy`; the runtime alone updates its ConfigMap and reloads.
...
| Unit | Content |
|---|---|
| 1 | `appa-adapter-kagent` Rust adapter crate: the canonical tool id and spawn derivation from the structured tool spelling, the `Adapter` enum variant, unit tests of the derivation |
...
| 3 | Python entrypoint: strict config schema and refusal rules, stock plugin parity, both config deliveries, and the controller args contract. Also the `reasoning_effort` fill from `APPA_KAGENT_OPENAI_REASONING_EFFORT`, and the runtime-owned remedy and battery matcher toolset with its 300 s request timeout |
...
| 6 | `appa-kagent-adk-go`: adk/v2 mapping verification, the Go plugin and runtime main, one image under two published names, the runtime-owned toolset |
...
- Replace path: `DeliverValue` and `ReplaceOutput` at the after-tool point — the admitted value reaches the model as it crossed, the runtime's own words reach it spelled back — a `ChildReturn` echoed at the stop of the child, and a `Block` as the withheld result at the parent.
...
- Human review (per plugin): a reviewed offer raises the confirmation before the control call crosses. The resumed call carries the ruling. Both plugins also pin the approval-turn bridge that emits one exact control call when a model tries to stop with one review pending. They do not bridge without explicit chat approval. The plugin never reports the review dict as a result.
...
- No link from the adapter crate to `appa-runtime` or `appa-engine` (`appa-adapter-kagent/Cargo.toml` depends on `appa-runtime-api` only), and no policy state in either plugin.
...
- The shared fixture [wire-events.jsonl](../../marketplace/plugins/kagent/fixtures/wire-events.jsonl) pins the wire. The python plugin and the Go plugin both read it, and both build every row from their own wire module.

Every kagent job in [.github/workflows/ci.yml](../../.github/workflows/ci.yml) runs only on its own inputs. A first job, `changes`, reads the changed paths with `dorny/paths-filter` and answers five questions: `python`, `go`, `chart`, `integration` and `e2e`. Each kagent job takes one answer as its condition. A commit outside a job's inputs runs none of it. The Rust workspace jobs stay unconditional, because they cover the adapter crate.
...
|---|---|---|
| [test_core.py](tests/test_core.py) | 8 | the ordinary read, the refused exfiltration, the sanitized default, a forged offer id, two gated injection reads, and both sides of the GitHub battery boundary |
...
- Integration suite ([tests/](tests/)): twenty-four matrix cases on the real gated path, with a scripted model and no cluster. The real runtime, the real tools, the real mock externals, and both agents built by the real entrypoint. [automated: `integrations/kagent/tests` under `APPA_INTEGRATION=1`, a pull-request gate]
...
## Held batch proxy protocol

`GET /capabilities` advertises `held_batches`, `position_bound_batch_offers`, `batch_commit`, and `input_rewrite_holds_dispatch`. These events use the existing UUID `event_id` envelope.

`prepare_batch` takes a proxy UUID `batch_id` and ordered `calls`. The runtime supplies that UUID to the core proposal batch. Its response is ordered by `position`, carries typed offers per blocked position, and never returns a business `dispatch_id`.

`resolve_batch_offer` has the ordinary offer fields plus `batch_id` and `position`. The proxy checks the durable actor, core batch, position, original/effective call, offer, and deployment mapping. A signed human grant also includes and signs `batch_id` and `position`; it is distinct from the unsigned model remedy selection. An input sanitizer records its canonical effective arguments and holds its resulting dispatch instead of returning a client call.

`commit_batch` takes `root_id`, optional `child_id`, and `batch_id`. It refuses until every position is authorized. It opens any approval-backed dispatches inside the proxy, then returns one ordered `batch_committed` release. Every release contains its exact effective arguments and `spawn_binding` when the prepared position had `spawn: true`; the latter is the core-issued fork identity and must be preserved when reporting the child outcome. Only those returned dispatch mappings can receive tool results. A completed envelope replays. A pending receipt or failed durable mapping quarantines its root and prevents authority, sanitizer, or business-call retry.
