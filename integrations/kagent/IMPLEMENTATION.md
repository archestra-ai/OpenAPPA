...
- **Tool naming and coverage.** An annotation pins to the canonical digest under the canonical tool id. The `[[tool]]` entries and mandates must match that id for every toolset. The entrypoint builds a tool inventory from the rendered config at startup and spells each tool on the wire in a structured form; the runtime derives the canonical id from it. An MCP tool of the server `demo-tools` crosses as `mcp:demo-tools/<tool>` and is named `mcp/demo-tools/<tool>`; the toolset is the first DNS label of the server host in the rendered URL, the name of the `RemoteMCPServer` resource. An agent tool, which kagent dispatches as `<namespace>__NS__<agent name>` with underscores, crosses as `agent:<namespace>/<agent name>` and is named `agent/<namespace>/<agent name>` with the real hyphens. A tool the kagent runtime attaches itself (`ask_user`, the memory tools, the skills tools) crosses as `builtin:<name>` and is named `host/kagent/<name>`; the out-of-band gates cross as `gate:code_execution` and `gate:memory_persist` and are named `host/kagent-gate/<name>`. A gated agent must list its MCP tools: an MCP entry without a tool filter refuses the start, and a call of a name outside the inventory is refused at the gate. The wildcard entry (`name = "*"`) is the recommended first posture for a fleet. CRD-declared toolsets produce a long tail the policy never names in advance. No tooling generates `[[tool]]` entries. Write them from the `Agent` and `RemoteMCPServer` resources by hand. The reserved `execute_remedy_plan` needs no entry — the runtime recognizes its own tool first.
...
- The `demo-tools` Deployment, Service, and `RemoteMCPServer` ([demo/Dockerfile](demo/Dockerfile)). Two canned public-GitHub tools, `mcp__github__get_file_contents` and `mcp__github__issue_write`, are intentionally absent from the fixture policy template. Their qualified wire names exactly match the shipped GitHub battery declarations, so `appa-guide` proposes the include without adding shadowing root contracts.
...
- The shared fixture [wire-events.jsonl](../../marketplace/adapters/kagent/fixtures/wire-events.jsonl) pins the wire. The python plugin and the Go plugin both read it, and both build every row from their own wire module.
