...
The quickstart is optional and orthogonal to everything above — same plugins, same wire, same python runtime layer as `appa-kagent-adk`. Skipping it changes nothing. It exists so one operator can gate one agent in minutes, with no separate `appa-runtime` deployment.
...
- ModelConfig `appa-demo-model` over the Secret of the same name (`openai.apiKey` or `openai.existingSecret`). So the Models → Edit flow of the dashboard supplies the key after install. The default matches the public playground: `openai/gpt-5.6-luna` at `https://openrouter.ai/api/v1`.
