...
- The `appa-guide` Agent: packaging around the routing skill at [skills/appa-guide](../../skills/appa-guide), attached through kagent git-ref skills, with the kagent tool server's `k8s_*` tools and the shared runtime as its own gate. The skill inventories `RemoteMCPServer.status.discoveredTools` and `Agent` tool declarations, proposes contracts, and applies the policy ConfigMap through `k8s_apply_manifest` — which the demo policy puts behind `human-approval`, so the apply raises kagent's Approve card — then waits for the mount sync and POSTs `/reload` through `k8s_execute_command`.
