// Runner assembly adapted from kagent af84a618cb6b go/adk/pkg/runner/adapter.go (Apache-2.0).
// Delta: pass invocation-owned MCP discovery as an ADK request processor while
// retaining the stock agent, model, memory, share tools, sessions and STS plugin.
...
func CreateDiscoveryRunnerConfig(
	ctx context.Context,
...
) (runner.Config, *MCPDiscovery, error) {
	...
	discovery, err := newMCPDiscovery(agentConfig, stsPlugin)
	if err != nil {
		return runner.Config{}, nil, err
	}
