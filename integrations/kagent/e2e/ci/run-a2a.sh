...
#   APPA_E2E_CASES                    optional pytest -k expression
...
wait_for_url() {
  ...
  echo "run-a2a: $label never answered at $url" >&2
...
case_args=(.)
case_label="all 17 A2A conversations"
if [ -n "$cases" ]; then
  case_args=(-k "$cases" .)
  case_label=$cases
fi

echo "== A2A cases '$case_label' on $model at $base_url"
started=$(date +%s)
status=0
(
  cd "$a2a"
  ...
  APPA_A2A_E2E=1 APPA_A2A_URL="$agent_url" APPA_MOCK_URL="$mock_url" \
    ...
    pytest -v -rA --durations=0 --reruns 2 "${case_args[@]}"
) 2>&1 | tee "$log" || status=$?
