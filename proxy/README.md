...
Puts OpenAPPA on the **inference layer**: an OpenAI-compatible HTTP proxy that
sits between an agent harness and the LLM and blocks tool calls that fail their
OpenAPPA contract, before the harness ever sees them — no changes to the harness
beyond pointing its `base_url` at the proxy.

The proxy keeps no state. On every `/v1/chat/completions` response it rebuilds
the OpenAPPA [`Trajectory`](../core) from the request `messages`, evaluates
...
parsed by [`appa-contracts`](../contracts) into `ToolContract`s.
...
```sh
cargo run --bin appa-proxy -- --policy policy.toml --addr 127.0.0.1:8730
