...
//! unknown-handling is authority registration in current appa-core. Inline
//! `allow` authorities rule in-process; an `escalate` (external) authority
//! must declare a `webhook` endpoint — the proxy's one approval channel. An
//! escalate authority without one could never rule: every call routed to it
//! would block at runtime with a confusing "needs approval" dead end, so the
//! config fails at startup instead. Flows no declared authority covers fail
//! closed.
...
mod tests {
    ...
    fn kagent_demo_policy_loads() {
        // The demo's policy file is outside the cargo battery's runtime
        // reach; pin here that it stays loadable as the dialect evolves.
