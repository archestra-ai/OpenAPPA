...
//! session feed and renders typed verdicts back into proxy wording. The
//! proxy has no approval channel, so verdicts are checked with
//! [`NoResolver`]: every escalation to an external authority stays blocked.
