...
| `host/claude-code/Bash(command:*databricks *)` | A Databricks CLI command is classified in the `databricks` battery's vocabulary: `databricks experimental aitools tools query` by the SQL it carries, every other command by what it does. A read returns `suspicious` `internal` data; a change needs `trusted` input and records `databricks.changed`; DDL, grants, several statements, SQL from a file or stdin, permissions, secrets, or anything unclear needs the `databricks-review` mark and records `databricks.sensitive`. The model reads the whole command line, chained commands included. `.databrickscfg`, `auth token`, `auth env`, `secrets get-secret`, and `configure` are credential commands, narrowed to `self` before any classifier runs. |
