...
The runtime uses the current user's Claude Code authentication. For each consult, it starts a fresh safe-mode process with no tools, hooks, project settings, or persisted session in a temporary working directory, stripping all `APPA_*` environment variables. The system prompt carries the Annotator's trusted `hint`, input mapping, and mandate vocabulary. The artifact is the sole user turn, always including the tool name and policy description (when present). If the Annotator maps no inputs, `args` contains the complete argument object; otherwise, it contains only the mapped values under their aliases. No trajectory state—such as current labels or history—is transmitted. The Annotator returns a complete annotation in a single consult, establishing output labels, requirements, and emitted effects. At most four Claude consults may run concurrently.
