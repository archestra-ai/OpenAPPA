...
Each consult sends the tool's name, description, and arguments to that endpoint. OpenAPPA first replaces known secret shapes and the value of any field named for a secret, such as `password` or `Authorization`, and cuts each string at 4,000 characters. A slow request is repeated on a new connection, and a server error or a connection failure is retried, within `timeout_ms`. The consult record carries the attempts and the label probabilities under `jev_diagnostics`.
