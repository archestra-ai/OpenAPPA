...
A host that embeds the runtime brings its own tool identification. The adapter identifies every call's canonical id. The runtime spells a tool back through the same adapter. See [Writing an Integration](/writing-an-integration#how-the-hooks-reach-appa).
