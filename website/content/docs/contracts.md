...
| **Input sanitizer over a resolver** | A `tool_input` sanitizer in scope of a tool whose resolver classifies the very arguments it rewrites. | Scope the sanitizer to tools whose resolver classifies the proposal, or drop the `tool_input` point. | The rewrite keeps the answer given before it. A resolver that would have classified the rewritten arguments differently is not consulted again. |
