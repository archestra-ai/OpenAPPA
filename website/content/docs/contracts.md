...
Two configurations avoid that, and the policy makes the choice, not the sanitizer:

- Map the resolver's `inputs` to arguments the sanitizer does not rewrite. A resolver that reads `$tool_call.arguments.customer_id`, beside a redactor that rewrites `body`, classifies a value the rewrite never touches.
- Or keep `tool_input` sanitizers out of the tool's `scope` tags. A resolver that reads `$tool_call`, `$tool_call.arguments`, or the argument the sanitizer rewrites is classifying the bytes that change, so the tool takes no input sanitizer.
...
| **Input sanitizer over a resolver** | A `tool_input` sanitizer in the `scope` tags of a tool whose resolver reads `$tool_call`, `$tool_call.arguments`, or an argument that sanitizer rewrites. | Map the resolver's `inputs` to arguments the sanitizer does not rewrite, or leave the tool out of the sanitizer's `scope` tags. | The rewrite dispatches under the answer given before it: a rewrite that changes a recipient, path, or resource carries the classification of the original one. The resolver is not consulted again. |
