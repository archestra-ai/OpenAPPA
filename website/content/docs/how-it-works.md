...
A tool can also run dynamic resolvers over its complete arguments before dispatch. Each resolver separately declares output `delta` fields and call-time `requires` fields. It can narrow output trust/audience while also returning a trust floor, audience `includes`/`cap` constraints, and attention marks for that call.

Each dynamic resolver binds independently: HTTP implementations use `[externals.dynamic.<resolver-name>]`, while a tool-level resolver may declare `builtin = "claude-code"` inline. Every tool-level request carries the policy's trust chain and the attention marks named by authority mandates. Trust answers must select a rank from that chain, and attention answers must select literal marks from that attended set, preserving per-mark authority routing. The builtin runs a fresh, tool-less Sonnet classification through the local Claude Code CLI and can return `delta` and `requires` together. It receives the canonical request plus current label and static attention context; failures or out-of-policy values produce no evidence and stop the check. This experimental classifier has no mandate or ceiling of its own, so its fixed prompt and returned evidence are part of the trusted deployment boundary.
