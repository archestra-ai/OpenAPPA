...
A runtime checkpoint preserves a root's Label, completed policy effects and denials. It excludes pending dispatches, offers and live approvals. A fork from that checkpoint creates a separate root without replaying tool execution. It does not copy raw Values or replace the integration's model-context handling. See [Runtime checkpoints](/runtime-checkpoints) for the authenticated proxy API and its boundaries.
