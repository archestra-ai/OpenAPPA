...
- **Annotator**: Where the contract depends on the call, the `[[tool]]` entry names a registered **annotator**. The annotator reads selected call data and answers the complete contract inside its **mandate**.

An Annotator declaration can include a trusted policy `hint`. The hint defines policy-specific values and the evidence that selects them. It cannot expand the mandate. Every artifact identifies the proposed tool and carries its policy description, when present. An input mapping can limit which argument values cross the consult boundary.
