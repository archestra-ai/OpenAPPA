...
- **[`appa yell` reports](/yell-v2):** feedback linked to the decisions that prompted it.
...
Your agent can periodically review decisions and [`appa yell` reports](/yell-v2) in a “dream” cycle. For a task that repeatedly gets blocked, it can [propose a change to `appa.toml`](/self-improving-policies-v2#give-the-maintenance-agent-a-separate-job) and test it against allowed and prohibited calls.
