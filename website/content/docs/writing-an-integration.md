...
|---|---|---|---|---|
| 1 | **`session_start`** | Root conversation or agent task initializes. | `root_id`, a stable root id from the harness session id, and optionally `principal`: the email address of the authenticated person the session acts for (see [Session principal](/contracts#session-principal)). A malformed principal, or one that differs from the principal the session already opened for, is refused. | `ack`: Continue session startup.<br>`refuse`: Abort session startup and display the returned `detail` message. |
