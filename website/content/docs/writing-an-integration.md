...
A host that embeds the runtime in its own process skips the wire altogether. Archestra does: it opens the runtime with `Runtime::open_with_store_as` and the [`appa-adapter-archestra`](https://github.com/archestra-ai/OpenAPPA/tree/main/appa-adapter-archestra) adapter, composes its policy from a root document and the batteries it installed with `Config::hosted_composed`, and derives every call through the adapter before handing the event over, so battery rules written canonically reach the catalogs the host aliased to them.

A new host chooses one of these shapes: build the envelope in-process, as kagent does, translate a host's hook format in a client, as `appa hook` does, or embed the runtime, as Archestra does. Either way it needs an adapter crate the runtime is built with, because the runtime derives the tool identity of every call from it. For a complete reference, see [`appa-adapter-claude-code`](https://github.com/archestra-ai/OpenAPPA/tree/main/appa-adapter-claude-code), [`appa-adapter-kagent`](https://github.com/archestra-ai/OpenAPPA/tree/main/appa-adapter-kagent) and [`appa-adapter-archestra`](https://github.com/archestra-ai/OpenAPPA/tree/main/appa-adapter-archestra); the envelope and decision types are in [`appa-runtime-api`](https://github.com/archestra-ai/OpenAPPA/tree/main/appa-runtime-api).
