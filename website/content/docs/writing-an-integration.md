...
- **`POST /proxy/v1/checkpoints`**: Bearer-authenticated proxy API for a durable root checkpoint and an independent root fork. `GET /proxy/v1/openapi.json` is its generated machine-readable contract. See [Runtime checkpoints](/runtime-checkpoints) for validation and state-copy limits.
